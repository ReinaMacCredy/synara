# Plan 006: Make Veylen the Authoritative Agent Harness
...
An explicit request to start two Veylen threads produced six threads. The
...
`veylen_create_thread`, tried several invalid provider/model combinations, and
...
1. **Model awareness** — every provider is told, through its strongest native
   instruction channel, that Veylen is the hosting harness and that Veylen
   resources are controlled through `veylen_*` tools.
2. **Capability truth** — agents can ask Veylen which providers, models,
...
```text
user asks for exactly two Veylen threads
  -> parent reads veylen_context / veylen_capabilities
  -> parent calls veylen_create_threads once with exactly two entries
  ...
  -> parent calls veylen_wait_for_threads until both runs are terminal
...
- Veylen is the harness; provider-native subagents are implementation details
  inside a provider turn and never substitute for user-requested Veylen
  threads.
- All model-facing host text comes from one versioned
  `VeylenHarnessPolicy`. Provider adapters only choose the delivery mechanism.
- Every state-changing gateway call is bound to a live Veylen provider session
...
- Unsupported provider/model/options fail before `thread.create`, git branch,
  worktree, or provider side effects. Veylen never interprets `Low` as part of
...
- `apps/server/src/agentGateway/Layers/AgentGateway.ts:73-81` owns the only full
  “Veylen is hosting this session / use veylen\_\*” policy, but
...
- `apps/server/src/provider/Layers/ClaudeAdapter.ts:880-885` tells Claude it is
  running inside Veylen, but does not define Veylen-thread versus native
  subagent precedence.
- `apps/server/src/codexAppServerManager.ts:475-486,620-648` sends Codex
  ...
  include the Veylen harness contract.
- Cursor, Grok, and Droid receive MCP transport configuration and a
  Veylen `clientInfo`, but no shared model-facing host policy. See
...
- `apps/server/src/provider/runtimeLayer.ts:54-70` explicitly excludes OpenCode
  ...
  and a clear “Veylen control unavailable” capability instead of pretending
  parity.
...
- A background native agent that retains the same MCP connection can therefore
  mutate Veylen after the visible parent turn has completed.
...
- predecessor naming — Veylen identity must remain consistent;
...
### `veylen_context` (read-only, idempotent)

```ts
type VeylenContextResult = {
  harness: { name: "Veylen"; policyVersion: string };
  caller: {
    threadId: ThreadId;
    turnId: TurnId | null;
...
### `veylen_capabilities` (read-only, idempotent)
...
### `veylen_create_threads` (destructive, idempotent with request id)

```ts
type VeylenCreateThreadsInput = {
...
Keep `veylen_create_thread` as a backwards-compatible single-entry wrapper
...
### `veylen_wait_for_threads` (read-only, idempotent)

```ts
type VeylenWaitForThreadsInput = {
...
- Rebranding or renaming `veylen_*` tools.
- Disabling provider-native subagents globally. They remain available for
  ...
  explicit Veylen-resource requests.
...
- UI redesign. Existing Veylen MCP logo/labels may consume tool annotations,
...
- Branch from the current feature branch with `codex/veylen-harness-control-plane`.
...
### Step 2: Centralize and deliver the Veylen harness policy

Create a pure `harnessPolicy.ts` exporting:

- `VEYLEN_HARNESS_POLICY_VERSION`;
...
1. “You are running inside Veylen. Veylen is the host/harness for this
   session.”
2. Use `veylen_*` for Veylen threads, projects, automations, and coordination.
3. Provider-native `spawn_subagent`/Task/collaboration tools do not create
   Veylen threads and must not substitute for an explicit request for Veylen
   threads.
4. Use one `veylen_create_threads` call for plural requests; the array count is
   exact.
5. Resolve canonical models/options through `veylen_capabilities`; do not guess
   slugs or silently change provider/model.
6. Wait with `veylen_wait_for_threads`, then synthesize all results.
...
- **Claude**: append it to the existing Claude system-prompt append. Add MCP
  ...
  list.
- **Cursor/Grok/Droid ACP paths**: include consistent Veylen clientInfo
  ...
  user-visible Veylen message projection.
- **OpenCode/Kilo**: deliver truthful host identity, but advertise Veylen MCP
...
- **Pi**: deliver host identity through its supported prompt/extension path and
  explicitly advertise that Veylen MCP control is unavailable. Do not simulate
  successful Veylen actions.

Keep MCP initialize `instructions` generated from the same policy. Add tool
annotations and make every description begin with its user-facing Veylen
...
- caller thread id;
- a Veylen-generated provider-session key (not an upstream provider id);
- provider kind;
...
- return the same catalog from `veylen_capabilities` that validation uses;
...
Implement `veylen_wait_for_threads` as a read-only long poll over projections:
...
This cap protects Veylen projections from provider fan-out, but it does not
disable native subagent execution. Keep native-child budgeting separate from
the ordinary top-level `veylen_create_threads` operation.
...
- the active parent path uses one two-entry Veylen batch;
...
- [x] Every provider receives the same versioned Veylen host identity through
      its strongest available instruction channel.
- [x] Every provider truthfully reports whether it can call Veylen MCP; no
      provider simulates unsupported host actions.
- [x] MCP initialize identity, tool descriptions, and native host instructions
      all say Veylen, never a predecessor or provider-branded harness.
- [x] Every destructive call requires a live scoped provider session and active
      caller turn.
- [x] A detached background agent cannot create or mutate Veylen threads after
...
- `VeylenHarnessPolicy` is the single source of truth. Any new provider must add
