...
- Publishes default `latest*.yml` metadata plus byte-identical `veylen*.yml` aliases on every stable release so existing packaged binaries keep working.
...
- Publishes prerelease installers only on their versioned GitHub prerelease; prereleases never replace the stable `veylen` update manifests.
- Publishes the CLI package (`apps/server`, npm package `@veylen/cli`) with OIDC trusted publishing.
...
- Runtime channel: `veylen`. Stable 0.5.x releases publish both `latest` and `veylen` metadata; the 0.4.x compatibility release remains available for historical migration.
- Repository slug source:
  - `VEYLEN_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Required Veylen release assets for updater:
  - platform installers (`.exe`, `.dmg`, `.AppImage`, plus macOS `.zip` for Squirrel.Mac update payloads)
  - `veylen-mac.yml`, `veylen.yml`, and `veylen-linux.yml` metadata
  - every stable release includes both `veylen-mac.yml`, `veylen.yml`, `veylen-linux.yml` and `latest-mac.yml`, `latest.yml`, `latest-linux.yml`
  - `*.blockmap` files, except the macOS update `.zip.blockmap` removed after zip repack
- Enforced upgrade path:
  - Stable clean Veylen releases are created with `make_latest=true` and carry both six-manifest filenames in the versioned release.
  ...
  - Clean-release publication fails closed if either the default Latest manifests or the dedicated `veylen` aliases are missing.
- Production desktop builds omit web/server/desktop source maps by default to keep update payloads small. Set `VEYLEN_WEB_SOURCEMAP=1`, `VEYLEN_SERVER_SOURCEMAP=1`, or `VEYLEN_DESKTOP_SOURCEMAP=1` only for a diagnostic release that needs them.
- macOS metadata note:
  - The build initially emits `latest-mac.yml` for both Intel and Apple Silicon.
  - The workflow merges the per-arch macOS metadata, then keeps the merged manifest as `latest-mac.yml` and copies it to `veylen-mac.yml` for stable releases.
...
1. Confirm the npm account controls the `@veylen` scope and can publish `@veylen/cli`.
...
- The published release title should read `Veylen vX.Y.Z`.
...
- Optional jobs stay disabled unless repository variables enable them:
  - `VEYLEN_PUBLISH_CLI=1`
  - `VEYLEN_FINALIZE_RELEASE=1`
...
8. Confirm the new versioned release is not GitHub Latest and the pinned compatibility release contains the new payloads plus all three `veylen` manifests.
