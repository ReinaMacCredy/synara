...
- Electron owns one main process, one primary browser renderer, Chromium GPU/utility
  processes, and one Veylen backend child. These are application infrastructure,
...
belong to that provider. Veylen's additional cost is its Electron/backend process
...
The page exposes `window.__veylenPerf.snapshot()`, `resetMetrics()`,
...
```sh
env -u VEYLEN_AUTH_TOKEN VEYLEN_PORT_OFFSET=4117 VEYLEN_NO_BROWSER=1 \
  bun run dev -- --home-dir ./.veylen-perf-audit --port 59231
```
...
| Scenario                                             | Veylen CPU / memory                                                                           | Provider CPU / memory                                                              | Processes                                                               | Renderer/frame evidence                                                    | Observation                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Packaged backend, five-second sample                 | 269 MB physical, 387 MB peak; 4,110/4,211 main-thread samples in `kevent`                     | Not included                                                                       | One Veylen backend                                                      | n/a                                                                        | The observed 7.9% was transient, not sustained backend CPU.                                                                                                                                                                                                                                           |
...
| Equivalent task through Veylen                       | About 93 MB dev backend at the sampled instant                                                | Approximately 156 MB for the provider pair, approximately 0% CPU after completion  | Veylen backend/browser plus provider tree                               | Response completed in 23 seconds                                           | The provider RSS was essentially the same as direct; Veylen adds its own backend/browser processes and event pipeline. Model differed (GPT-5.5 through Veylen versus GPT-5.6 direct), so wall time is not a valid latency comparison.                                                                 |
...
### 1. Durable journal safety polling (medium/high idle impact, Veylen-owned)
...
### 2. Unrelated streaming updates scanned retained thread entries (medium under concurrency, Veylen renderer-owned)
...
### 3. Infinite ChatView working shimmer (medium steady renderer impact, Veylen-owned; retained by design)
...
### 4. Claude lifecycle lock map retained every historical thread (low per thread, unbounded, Veylen-owned)
...
### 5. Per-event durable journal cost (medium/high while streaming, Veylen-owned; measured but not changed)
...
a transaction. This maps provider chunk frequency directly to Veylen SQLite work.
...
- Shutdown PID audit found zero remaining descendants from the isolated Veylen tree.
- `bun fmt`: passed.
