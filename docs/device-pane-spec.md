...
| Helper distribution | Compile on-device at first attach with the user's Xcode toolchain; cache keyed by Xcode version (e.g. `~/Library/Caches/veylen/device-helper/<xcode-build>/`). Compile failure is a designed error state in the pane. References: Codex++ `sim-capture.swift`/`sim-input.m`, Meta's idb.                                                                              |
...
| Lifecycle           | Thread-scoped attachment (mirrors ThreadBrowserState). Veylen-booted devices shut down on app quit, thread archive, or idle timeout after detach. User-booted devices (pane picker or Simulator.app) are never auto-shutdown. Cap Veylen-booted devices at 3 globally; boot requests beyond that prompt to shut one down. Viewing already-booted devices is uncapped. |
...
| Upstream            | One complete feature PR from `aristotl-dylan/veylen`, feature-complete end to end.                                                                                                                                                                                                                                                                                    |
