...
You are GPT-5.5 (xhigh) in the Veylen monorepo at `/Users/emanueledipietro/Developer/veylen`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `VEYLEN_HOME` (dev: `./.veylen/electron-dev`; prod: `~/.veylen` or
  `~/Documents/Veylen`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `VEYLEN_HOME/VEYLEN_HOME/VEYLEN_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.veylen/electron-dev/dev/state.sqlite` (dev) and `~/.veylen/userdata/state.sqlite`
     (prod) where relevant.
