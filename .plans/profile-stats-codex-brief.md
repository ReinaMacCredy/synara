...
You are GPT-5.5 (xhigh) working in the Veylen monorepo at `/Users/emanueledipietro/Developer/veylen`.
...
- BRANCH dev DB (what the dev instance uses): `./.veylen/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.veylen/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@veylen/contracts --filter=@veylen/cli` must pass. DO NOT run the web
   (`@veylen/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
