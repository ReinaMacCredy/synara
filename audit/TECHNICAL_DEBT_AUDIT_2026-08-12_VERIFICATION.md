...
| ----------------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `bun fmt`                                                                                                         | PASS           | Post-CI-harness rerun formatted 2,806 files; `bun run fmt:check` then confirmed all 2,806 files.                                                                                                                                                                                           |
| `bun lint`                                                                                                        | PASS           | Post-CI-harness rerun completed across 2,696 files with 0 errors and 535 warnings.                                                                                                                                                                                                         |
| `bun typecheck`                                                                                                   | PASS           | Final rerun passed 7/7 workspace tasks in 46.297s.                                                                                                                                                                                                                                         |
...
| macOS arm64 DMG package command                                                                                   | PASS           | Final rerun emitted a 229 MiB DMG, 232 MiB ZIP, `latest-mac.yml`, and builder diagnostics under `/private/tmp/veylen-audit-artifact-ship-final-2`; SHA-256 values are `00200163...4b1de01` and `794275e1...fe607b45`.                                                                      |
| `bun run test`                                                                                                    | PASS           | Authoritative unrestricted final rerun: 8/8 Turbo tasks passed in 16m0.507s. The CLI/server corpus reported 399 passed files, 3 skipped files, 3,976 passed tests, and 16 skipped tests; the web corpus reported 337 files and 3,919 tests.                                                |
| `env VITEST_BROWSER_API_PORT=52100 bun run --cwd apps/web test:browser:stable`                                    | PASS           | 59 files; 292 passed and 11 skipped tests in 405.09s. Port 51100 was owned by a separate checkout, so the repository-supported override isolated this worktree without interrupting unrelated work.                                                                                        |
| `env VITEST_BROWSER_API_PORT=52101 bun run --cwd apps/web test:browser:geometry`                                  | PASS           | Blocking geometry selection: 1 passed and 58 skipped files; 11 passed and 292 skipped tests in 205.95s.                                                                                                                                                                                    |
| `bun run --cwd apps/web test:electron:e2e`                                                                        | PASS           | Final local rerun passed both native tests in 29.1s. The annotation path also passed 5/5 repeated real-Electron runs in 34.8s after picker focus became explicit state rather than Xvfb-dependent `activeElement`; ambient input filtering preserves the deliberate manual-takeover proof. |
| `bun run test:desktop-smoke`                                                                                      | PASS           | Desktop smoke launched the packaged application and passed all 5 graph tasks in 16.934s.                                                                                                                                                                                                   |
| `bun run release:smoke:mac-update -- --artifact-dir /private/tmp/veylen-audit-artifact-ship-final-2 --port 58159` | PASS           | ZIP and update manifest were served and validated; `Content-Length` matched 242,861,981 bytes, and obsolete blockmap removal was confirmed.                                                                                                                                                |
...
| Desktop packaging         | PASS      | The final macOS arm64 command completed and emitted a 229 MiB DMG, 232 MiB ZIP, `builder-debug.yml`, and `latest-mac.yml` to `/private/tmp/veylen-audit-artifact-ship-final-2`. Focused publish-config tests (3), mac config tests (8), scripts typecheck, and mac update smoke pass.        |
...
The first full PR run, GitHub Actions run `31595089874`, passed migration lineage, release smoke,
Windows process regression, PR policy, and every aggregate step through desktop/preload build. Its
only failure was Electron launch on the Linux runner: Electron rejected GitHub's `chrome-sandbox`
because that helper is not root-owned with mode 4755. This was a CI harness gap, not an application
assertion failure. The shared Electron fixture now supplies `--no-sandbox` only on Linux; macOS and
Windows retain Electron's default sandbox behavior, and a two-case regression test locks that policy.

Run `31597618084` then reached the full unit corpus and exposed a transferred-turn checkpoint race;
commit `0c2497035` restores the missing baseline before comparison and its 31-test focused reactor
suite passes. Run `31599130868` initially encountered a corrupt runner TypeScript cache; its rerun
passed every gate through browser geometry, desktop smoke, release smoke, and preload checks, then
exposed Linux-specific closed-shadow focus traversal and ambient native input in Electron E2E. The
final fixes explicitly own annotation-picker Tab focus and let the shell filter ambient keyboard and
pointer input without consuming expected automation or the deliberate human-takeover assertion.

Run `31606400827` passed Windows process regression, migration lineage, release smoke, the full unit
corpus, stable and geometry browser suites, desktop build, and preload verification. Electron's
visible-Browser MCP scenario passed, but Xvfb again left the closed-shadow annotation picker without
a committed event after native `Tab` and `Enter`. The remaining root cause was the preload's reliance
on `shadowRoot.activeElement`, which is not a stable keyboard-state authority under Linux/Xvfb. The
picker now owns a two-control focus state, synchronizes it from the DOM when available, and otherwise
advances it from trusted native Tab events. Five repeated annotation E2E runs, the complete two-test
Electron suite, and desktop typecheck pass after that correction.
