# Veylen Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Veylen for lifecycle, cancellation, queues, errors, and application policy.
...
| Commit              | Summary                                       |                                         Size |
| ------------------- | --------------------------------------------- | -------------------------------------------: |
| `0e5f6af9c`         | Refactor Veylen orchestration and web UI flow |                  387 files, +43,323 / -9,299 |
...
AcpSessionRuntime
  ...
  └─ Veylen: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
| Workstream                              | Why it was needed                                                                   | Main change                                                             | Result to verify                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `P2-ACP-01` ACP foundation              | Veylen maintained a custom wire stack beside the official SDK                       | Official SDK owns all production ACP wire behavior; custom wire deleted | Grok, Droid, and Cursor have one wire implementation and no fallback   |
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Veylen's application
lifecycle.
...
| Provider process supervision                  | Veylen/Effect          |
| Queue and resource limits                     | Veylen/Effect          |
| Session/product policy                        | Veylen/Effect          |
| Normalized Veylen events                      | Veylen/Effect          |
| Error translation into Veylen domain errors   | Thin local adapter     |
...
3. Provider adapters should depend on a stable Veylen runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Veylen. A
...
SQLite persistence, or the full Veylen app.
...
```text
Review the Veylen branch `codex/audit-code-quality` as a senior architecture, correctness,
