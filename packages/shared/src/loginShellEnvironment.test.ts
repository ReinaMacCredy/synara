...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "veylen-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Veylen home.
    const env = { VEYLEN_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Veylen home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { VEYLEN_HOME: "/tmp/veylen-home" } })).toBe(
      Path.join("/tmp/veylen-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".veylen", "cache", "login-shell-environment.json"),
    );
  });
});
