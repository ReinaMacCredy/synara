 ...
 */
export const WORKTREE_BRANCH_PREFIX = "veylen";
...
const VEYLEN_BRANCH_FALLBACK = "update";
...
export function buildVeylenBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || VEYLEN_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueVeylenBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildVeylenBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
