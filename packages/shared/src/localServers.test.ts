...
import type { ServerLocalServerProcess } from "@veylen/contracts";
...
describe("localServerPrimaryLabel", () => {
  it("prefers the live page title when one was resolved", () => {
    expect(localServerPrimaryLabel(makeServer({ pageTitle: "Veylen", displayName: "Vite" }))).toBe(
      "Veylen",
    );
  });
...
describe("localServerFolderLabel", () => {
  it("returns the final segment of a POSIX cwd", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/veylen-website" }))).toBe(
      "veylen-website",
    );
  });

  it("ignores a trailing separator", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/veylen/" }))).toBe(
      "veylen",
    );
  });
