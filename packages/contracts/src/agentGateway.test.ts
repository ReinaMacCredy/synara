...
import {
  VeylenCapabilitiesResult,
  VeylenCreateThreadsInput,
  VeylenCreateThreadsResult,
  VeylenGatewayErrorResult,
  VeylenWaitForThreadsInput,
  VeylenWaitForThreadsResult,
} from "./agentGateway";

const decodeCreate = Schema.decodeUnknownSync(VeylenCreateThreadsInput);
const decodeWait = Schema.decodeUnknownSync(VeylenWaitForThreadsInput);
...
describe("agent gateway contracts", () => {
  ...
  it("decodes typed capability, creation, wait, and error results", () => {
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(VeylenCapabilitiesResult)({
        targetConstruction: {
          codex: {
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(VeylenCreateThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(VeylenWaitForThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(VeylenGatewayErrorResult)({
