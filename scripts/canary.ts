// FILE: canary.ts
// Purpose: Maintains and launches an isolated, frozen Veylen Canary checkout.
...
export function resolveCanaryPaths(
  ...
  const home = Path.resolve(
    env.VEYLEN_CANARY_HOME?.trim() || Path.join(homeDirectory, ".veylen-canary"),
  );
  const cacheBase = env.XDG_CACHE_HOME?.trim() || Path.join(homeDirectory, ".cache");
  const source = Path.resolve(
    env.VEYLEN_CANARY_SOURCE?.trim() || Path.join(cacheBase, "veylen-canary", "source"),
  );
  return {
    home,
    source,
...
function assertManagedSourceIsClean(paths: CanaryPaths): void {
  ...
  if (status.length > 0) {
    throw new Error(
      `Veylen Canary's managed source has tracked local changes. Refusing to overwrite ${paths.source}.`,
    );
  }
}
...
function startCanary(paths: CanaryPaths): void {
  ...
  if (existingPid !== null && isRunning(existingPid)) {
    console.log(`Veylen Canary is already running (pid ${String(existingPid)}).`);
    return;
  }
  ...
  if (
    ...
    throw new Error("Veylen Canary is not built. Run `bun run canary:setup` first.");
  }
  FS.mkdirSync(paths.home, { recursive: true });
  const env = { ...process.env };
  delete env.VITE_DEV_SERVER_URL;
  delete env.ELECTRON_RENDERER_PORT;
  delete env.VEYLEN_AUTH_TOKEN;
  Object.assign(env, {
    VEYLEN_DESKTOP_FLAVOR: "canary",
    VEYLEN_DISABLE_AUTO_UPDATE: "1",
    VEYLEN_HOME: paths.home,
    VEYLEN_COMMIT_HASH: commit,
  });
  ...
  try {
    ...
    if (child.pid === undefined) {
      throw new Error("Veylen Canary failed to return a process id.");
    }
    child.unref();
    FS.writeFileSync(paths.pid, `${String(child.pid)}\n`, { mode: 0o600 });
    console.log(`Started Veylen Canary at ${commit.slice(0, 12)} (pid ${String(child.pid)}).`);
...
function rollbackCanary(paths: CanaryPaths): void {
  ...
  if (state?.previousCommit === null || state?.previousCommit === undefined) {
    throw new Error("Veylen Canary has no previous successful commit to restore.");
  }
  assertManagedSourceIsClean(paths);
  stopCanary(paths);
...
function printStatus(paths: CanaryPaths): void {
  ...
  console.log(`Veylen Canary: ${running ? `running (pid ${String(pid)})` : "stopped"}`);
