...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Veylen.app"), [
      "Veylen.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Veylen.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Veylen.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Veylen.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Veylen.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Veylen.app/Contents/Info.plist",
        "Veylen.app/Contents/Info.plist",
        "Veylen.app/Contents/MacOS/Veylen",
      ]),
      "Veylen.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Veylen-0.1.5-arm64.dmg",
        "Veylen-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Veylen-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Veylen-0.1.5-arm64.zip", "Veylen-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Veylen-0.1.5-arm64.dmg"]),
...
files:
  - url: Veylen-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Veylen-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Veylen-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Veylen-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Veylen-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Veylen-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Veylen-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Veylen-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Veylen-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Veylen-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
