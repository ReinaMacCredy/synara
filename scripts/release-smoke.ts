...
files:
  - url: Veylen-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
path: Veylen-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Veylen-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
path: Veylen-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  if (serverPackage.name !== "@veylen/cli") {
    throw new Error(`Expected CLI package @veylen/cli, got ${serverPackage.name ?? "<missing>"}.`);
  }
  const expectedBinaries = {
    veylen: "dist/index.mjs",
    "veylen-restore-migration-backup": "dist/restoreMigrationBackup.mjs",
  };
  if (JSON.stringify(serverPackage.bin ?? {}) !== JSON.stringify(expectedBinaries)) {
    throw new Error(
      "Expected the CLI to expose only the Veylen entry point and migration recovery binary.",
    );
  }
  if (VEYLEN_PRODUCTION_BUNDLE_ID !== "com.reinamaccredy.veylen") {
    throw new Error(`Unexpected production bundle ID: ${VEYLEN_PRODUCTION_BUNDLE_ID}.`);
  }
  if (VEYLEN_DESKTOP_UPDATE_CHANNEL !== "veylen") {
    throw new Error(`Unexpected desktop update channel: ${VEYLEN_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (
    ...
    throw new Error("Expected stable clean Veylen releases to publish on GitHub Latest.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.VEYLEN_PUBLISH_CLI == '1'",
    "Expected CLI publication to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.VEYLEN_FINALIZE_RELEASE == '1'",
    "Expected release finalization to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "VEYLEN_PUBLISH_RELEASE: ${{ needs.preflight.outputs.publish_release }}",
  ...
  assertContains(
    desktopBuildConfig,
    "__VEYLEN_WINDOWS_UPDATER_PUBLISHER__",
...
const tempRoot = mkdtempSync(join(tmpdir(), "veylen-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Veylen-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Veylen-9.9.9-smoke.0-x64.zip",
