...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Veylen\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Veylen.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    VEYLEN_HOME: join(root, "veylen-home"),
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "veylen");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `veylen-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.VEYLEN_HOME!, "userdata", "logs", "desktop-main.log");
