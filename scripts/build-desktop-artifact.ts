...
import { VEYLEN_PRODUCTION_BUNDLE_ID } from "@veylen/shared/desktopIdentity";
...
interface StagePackageJson {
  ...
  readonly veylenCommitHash: string;
  readonly veylenLockfileSha256: string;
  readonly veylenSourceTag: string | null;
  readonly veylenWindowsPublisherSubject: string | null;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "VEYLEN_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("VEYLEN_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "VEYLEN_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("VEYLEN_DESKTOP_VERSION").pipe(Config.option),
  sourceCommit: Config.string("VEYLEN_SOURCE_COMMIT").pipe(Config.option),
  sourceTag: Config.string("VEYLEN_SOURCE_TAG").pipe(Config.option),
  lockfileSha256: Config.string("VEYLEN_LOCKFILE_SHA256").pipe(Config.option),
  outputDir: Config.string("VEYLEN_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.string("VEYLEN_DESKTOP_SKIP_BUILD").pipe(Config.option),
  keepStage: Config.string("VEYLEN_DESKTOP_KEEP_STAGE").pipe(Config.option),
  signed: Config.string("VEYLEN_DESKTOP_SIGNED").pipe(Config.option),
  verbose: Config.string("VEYLEN_DESKTOP_VERBOSE").pipe(Config.option),
  mockUpdates: Config.string("VEYLEN_DESKTOP_MOCK_UPDATES").pipe(Config.option),
  mockUpdateServerPort: Config.string("VEYLEN_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
});
...
export const resolveBuildOptions = Effect.fn("resolveBuildOptions")(function* (
  ...
  const envSkipBuild = yield* resolveBooleanEnv("VEYLEN_DESKTOP_SKIP_BUILD", env.skipBuild);
  const envKeepStage = yield* resolveBooleanEnv("VEYLEN_DESKTOP_KEEP_STAGE", env.keepStage);
  const envSigned = yield* resolveBooleanEnv("VEYLEN_DESKTOP_SIGNED", env.signed);
  const envVerbose = yield* resolveBooleanEnv("VEYLEN_DESKTOP_VERBOSE", env.verbose);
  const envMockUpdates = yield* resolveBooleanEnv("VEYLEN_DESKTOP_MOCK_UPDATES", env.mockUpdates);
...
const verifyStagedNodePty = Effect.fn("verifyStagedNodePty")(function* (
  ...
  yield* runCommand(
    ChildProcess.make({
      ...
      env: {
        ...process.env,
        VEYLEN_NODE_PTY_SMOKE_REQUIRE_ROOT: stageAppDir,
      },
      ...commandOutputOptions(verbose),
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: VEYLEN_PRODUCTION_BUNDLE_ID,
    productName,
    artifactName: "Veylen-${version}-${arch}.${ext}",
    directories: {
...
const assertPackagedMacDeviceHelper = Effect.fn("assertPackagedMacDeviceHelper")(function* (
  ...
  for (const entry of entries) {
    const packagedAppRoot = path.join(stageDistDir, entry);
    const stat = yield* fs.stat(packagedAppRoot).pipe(Effect.catch(() => Effect.succeed(null)));
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `veylen-desktop-${options.platform}-stage-`,
  });
  ...
  const resolvedBuildConfig = yield* createBuildConfig(
    options.platform,
    options.target,
    desktopPackageJson.productName ?? "Veylen",
    options.signed,
    options.mockUpdates,
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    veylenCommitHash: commitHash,
    veylenLockfileSha256: resolvedLockfileSha256,
    veylenSourceTag: options.sourceTag ?? null,
    veylenWindowsPublisherSubject: resolvedBuildConfig.windowsPublisherSubject,
    private: true,
    description: "Veylen desktop build",
  ...
  if (options.platform === "mac") {
    yield* assertPackagedMacDeviceHelper(stageDistDir, desktopPackageJson.productName ?? "Veylen");
  }
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: VEYLEN_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: VEYLEN_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: VEYLEN_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: VEYLEN_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  sourceCommit: Flag.string("source-commit").pipe(
    Flag.withDescription("Expected full source commit (env: VEYLEN_SOURCE_COMMIT)."),
    Flag.optional,
  ),
  sourceTag: Flag.string("source-tag").pipe(
    Flag.withDescription("Exact source tag when building a release (env: VEYLEN_SOURCE_TAG)."),
    Flag.optional,
  ),
  lockfileSha256: Flag.string("lockfile-sha256").pipe(
    Flag.withDescription("Expected bun.lock SHA-256 (env: VEYLEN_LOCKFILE_SHA256)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: VEYLEN_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: VEYLEN_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: VEYLEN_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: VEYLEN_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: VEYLEN_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: VEYLEN_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.string("mock-update-server-port").pipe(
    Flag.withDescription("Mock update server port (env: VEYLEN_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Veylen."),
