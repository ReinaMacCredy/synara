...
- Dev commands default `VEYLEN_HOME` to `~/.veylen`, which keeps dev state under `~/.veylen/dev`.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.veylen-2`
...
- Desktop production windows load the bundled UI from `veylen://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `veylen` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `VEYLEN_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `VEYLEN_DEV_INSTANCE`)
- Example: `VEYLEN_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `VEYLEN_PORT_OFFSET` to a numeric offset.
