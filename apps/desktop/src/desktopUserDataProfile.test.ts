...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "veylen-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Veylen profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "veylen-dev" })).toBe(
      "/Users/tester/Library/Application Support/veylen-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "veylen" })).toBe(
      "/Users/tester/Library/Application Support/veylen",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "veylen-canary" }),
    ).toBe("/Users/tester/Library/Application Support/veylen-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "veylen");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "veylen-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "veylen-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Veylen profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Veylen database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "veylen");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".veylen-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "veylen");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "veylen-profile-seed.json"), "{");
