...
describe("desktopWsBridge", () => {
  ...
  it("reads only the canonical Veylen desktop URL environment value", () => {
    expect(
      resolveDesktopWsUrlFromEnv({
        VEYLEN_DESKTOP_WS_URL: "ws://127.0.0.1:6000/?token=veylen",
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
    ).toBe("ws://127.0.0.1:6000/?token=veylen");
    expect(
      resolveDesktopWsUrlFromEnv({
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
