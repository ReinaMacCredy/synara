...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    VEYLEN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    VEYLEN_CLAUDE_KEEPALIVE: "0",
    VEYLEN_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    VEYLEN_HOME: input.homeDir,
    VEYLEN_HOST: "127.0.0.1",
    VEYLEN_MODE: "desktop",
    VEYLEN_NO_BROWSER: "1",
    VEYLEN_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("veylen-windows-shutdown-");
