...
function cleanupStaleDevApps() {
  ...
  const commandPattern = `^${executable}[[:space:]]+--veylen-dev-root=${devRoot}([[:space:]]|$)`;
  spawnSync("pkill", ["-f", "--", commandPattern], { stdio: "ignore" });
}

function listStaleComputerUsePids() {
  // Only macOS exposes a verifiable Veylen (Dev) executable path for these
  ...
  return candidatePids.filter((pid) => {
    ...
    if (!/Veylen \(Dev\)\.app\/Contents\/MacOS\/Electron/.test(command)) {
      return false;
    }
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Veylen (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const pids = listPidsByExactProcessName("Veylen").filter((pid) =>
    readProcessCommand(pid).startsWith("/Applications/Veylen.app/Contents/MacOS/Veylen"),
  );
  if (pids.length === 0) {
    return;
  }
  ...
  console.error(
    "[desktop-dev] Veylen is still running. Close it before testing voice in Veylen (Dev), or you may be looking at the wrong app/runtime.",
  );
  console.error(`[desktop-dev] Running Veylen process IDs: ${pids.join(", ")}`);
}
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--veylen-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
