...
import { type ProjectId, type ThreadId, type TurnId } from "@veylen/contracts";
...
// Keep the v1 suffix stable while using the Veylen namespace; legacy
// `veylen:*` and `veylen:*` keys are copied over by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "veylen:split-view-state:v1";
