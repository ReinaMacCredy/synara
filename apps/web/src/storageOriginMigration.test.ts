...
describe("storageOriginMigration", () => {
  ...
  it("imports missing keys without overwriting current-origin state", async () => {
    globalThis.localStorage.setItem("veylen:theme", "current");
    const { importVeylenStorageSnapshot } = await import("./storageOriginMigration");

    expect(
      importVeylenStorageSnapshot({
        version: 1,
        exportedAt: "2026-07-09T00:00:00.000Z",
        entries: {
          "veylen:theme": "snapshot",
          "veylen:composer-drafts:v1": "draft",
        },
      }),
    ).toBe(true);
    expect(globalThis.localStorage.getItem("veylen:theme")).toBe("current");
    expect(globalThis.localStorage.getItem("veylen:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("rejects an invalid snapshot before writing any entry", async () => {
    ...
    expect(globalThis.localStorage.getItem("veylen:theme")).toBeNull();
  });
  ...
  it("imports snapshots containing large composer drafts", async () => {
    ...
    expect(globalThis.localStorage.getItem("veylen:composer-drafts:v1")).toBe(largeDraft);
  });

  it("migrates legacy Synara keys without overwriting Veylen state", async () => {
    globalThis.localStorage.setItem("veylen:theme", "current");
    const { importVeylenStorageSnapshot } = await import("./storageOriginMigration");
  ...
  it("keeps the snapshot retryable after a partial storage failure", async () => {
    ...
    const snapshot = {
      version: 1 as const,
      exportedAt: "2026-07-09T00:00:00.000Z",
      entries: { "veylen:theme": "dark", "veylen:composer-drafts:v1": "draft" },
    };

    expect(importVeylenStorageSnapshot(snapshot, storage)).toBe(false);
    storage.setItem = setItem;
    expect(importVeylenStorageSnapshot(snapshot, storage)).toBe(true);
    expect(storage.getItem("veylen:composer-drafts:v1")).toBe("draft");
  });
  ...
  it("acknowledges the desktop snapshot only after a complete bootstrap import", async () => {
    ...
    expect(globalThis.localStorage.getItem("veylen:theme")).toBe("dark");
  });
