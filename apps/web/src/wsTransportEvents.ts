...
import type { WsCompatibilityError } from "@veylen/contracts";

export type WsTransportState = "connecting" | "open" | "closed" | "incompatible" | "disposed";

export const VEYLEN_WS_TRANSPORT_STATE_EVENT = "veylen:ws-transport-state";
export const VEYLEN_WS_COMPATIBILITY_ISSUE_EVENT = "veylen:ws-compatibility-issue";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(VEYLEN_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(VEYLEN_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(VEYLEN_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
...
export function emitWsCompatibilityIssue(issue: WsCompatibilityError | null): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsCompatibilityIssueEventDetail>(VEYLEN_WS_COMPATIBILITY_ISSUE_EVENT, {
      detail: { issue },
    }),
  );
}
...
export function addWsCompatibilityIssueListener(
  ...
  window.addEventListener(VEYLEN_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  return () => {
    window.removeEventListener(VEYLEN_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  };
}
