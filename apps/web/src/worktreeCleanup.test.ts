...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.veylen/worktrees/veylen-mvp/veylen-4e609bb8",
    );
    expect(result).toBe("veylen-4e609bb8");
  });

  it("keeps legacy .veylen worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.veylen/worktrees/veylen-mvp/veylen-legacy123",
    );
    expect(result).toBe("veylen-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.veylen\\worktrees\\veylen-mvp\\veylen-4e609bb8",
    );
    expect(result).toBe("veylen-4e609bb8");
  });

  it("uses the final segment even when outside ~/.veylen/worktrees", () => {
