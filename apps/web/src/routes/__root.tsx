...
import { defaultTerminalTitleForCliKind } from "@veylen/shared/terminalThreads";
import { isThreadDetailEventFor } from "@veylen/shared/threadDetailEvents";
import {
  Outlet,
...
function TransportCompatibilityView({ issue }: { issue: WsCompatibilityError }) {
  const title =
    issue.action === "update-client"
      ? "This Veylen client needs an update."
      : issue.action === "update-server"
        ? "The Veylen server needs an update."
        : "Veylen needs to reconnect with a matching build.";
  const guidance =
    issue.action === "update-client"
      ? "Update or reload this client, then reconnect."
      : issue.action === "update-server"
        ? "Update or restart the server, then reload this client."
        : "Reload the app. If this repeats, restart Veylen so the client and server use matching builds.";
