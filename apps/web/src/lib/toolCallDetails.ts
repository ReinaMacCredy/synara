...
import type { ToolLifecycleItemType } from "@veylen/contracts";

type WorkLogRequestKind = "command" | "file-read" | "file-change" | "permissions" | "mcp-tool";
...
function extractToolOutputDetails(input: {
  ...
  const truncated = rawOutput?.truncated === true || data?.__veylenTruncated === true;
