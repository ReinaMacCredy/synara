...
describe("parseBareComposerLink", () => {
  it("returns the URL when the whole text is one bare link", () => {
    expect(parseBareComposerLink("https://github.com/ReinaMacCredy/Veylen")).toBe(
      "https://github.com/ReinaMacCredy/Veylen",
    );
  });
...
describe("describeLinkChip", () => {
  it("shortens GitHub pull request URLs to owner/repo#number", () => {
    expect(describeLinkChip("https://github.com/ReinaMacCredy/Veylen/pull/155")).toEqual({
      label: "ReinaMacCredy/Veylen#155",
      isGitHub: true,
    });
  });
