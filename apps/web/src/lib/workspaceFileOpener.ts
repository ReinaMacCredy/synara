...
import { isSupportedLocalPreviewFilePath } from "@veylen/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  workspaceRelativePathOf,
} from "@veylen/shared/path";
import { isScratchWorkspacePath } from "@veylen/shared/threadWorkspace";
...
const VEYLEN_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const VEYLEN_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveVeylenPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!VEYLEN_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${VEYLEN_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveVeylenPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
