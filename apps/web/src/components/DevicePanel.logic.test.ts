import type { DeviceDescriptor, DeviceUdid } from "@veylen/contracts";
...
describe("hardware button shortcuts", () => {
  ...
  it("leaves unrelated chords to the app", () => {
    ...
    // Cmd+Shift only maps Home; other Cmd+Shift chords stay with Veylen.
    expect(
  ...
  it("shows the picker when only the helper build is left", () => {
    ...
    const view = resolveDeviceAvailabilityView({
      ...
      steps: [
        ...
        { id: "build-device-helper", label: "Build the Veylen device helper", done: false },
      ],
    });
