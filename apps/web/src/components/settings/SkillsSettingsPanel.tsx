...
// catalog (~/.veylen/skills plus each provider's skills folder), shows which provider
...
import type { ProviderKind, ServerSettings } from "@veylen/contracts";
...
export function SkillsSettingsPanel() {
  ...
  const veylenSkillsDir = catalogQuery.data?.veylenSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Veylen skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Veylen's copy is the fallback."
          status={
            veylenSkillsDir ? (
              <code className="break-all text-[11px] text-muted-foreground">{veylenSkillsDir}</code>
            ) : null
          }
          control={
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Veylen could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Veylen skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
