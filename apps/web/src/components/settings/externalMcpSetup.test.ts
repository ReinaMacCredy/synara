...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add veylen --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Veylen.app/Contents/MacOS/Veylen server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Veylen home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user veylen -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Veylen.app/Contents/MacOS/Veylen server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Veylen home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { veylen: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.veylen).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Veylen app");

    expect(prompt).toContain('project named "Veylen app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "veylen mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add veylen");
    expect(prompt).toContain("claude mcp add --scope user veylen");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("veylen_overview");
