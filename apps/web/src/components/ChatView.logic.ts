...
import { normalizeModelSlug } from "@veylen/shared/model";
import { buildVeylenBranchName } from "@veylen/shared/git";
import { isGenericChatThreadTitle } from "@veylen/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@veylen/shared/terminalThreads";
import {
  type ChatMessage,
  type SessionPhase,
  type Thread,
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "veylen:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "veylen:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Veylen, then try again.";
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildVeylenBranchName(input.associatedWorktreeBranch ?? input.title);
}
