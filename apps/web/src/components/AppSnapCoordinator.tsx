...
// Purpose: Routes native macOS AppSnaps into the correct Veylen composer draft.
    ...
    async (capture: DesktopAppSnapCapture) => {
      ...
      if (resolvedTarget.kind === "existing") {
        ...
        if (result.threadId) {
          ...
          const focused = focusedTargetRef.current;
          if (!focused) throw new Error("Veylen could not create a task for this AppSnap.");
          target = focused;
          openChatThreadPage(target.threadId);
        }
      }
      ...
      if (!image) throw new Error(error ?? "Veylen could not attach the captured AppSnap.");
      ...
      toastManager.add({
        ...
        description:
          persistenceResult === "unverified"
            ? "The capture is attached, but Veylen could not verify its draft metadata. If it is missing after a reload, Veylen will attach it again."
