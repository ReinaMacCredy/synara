...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Veylen as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "veylen/fix-toast-copy");
  });

  it("normalizes an existing legacy veylen namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "veylen/refine-toolbar-actions");
    assert.equal(branch, "veylen/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Veylen", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "veylen/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "veylen/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "veylen/deadbeef";
