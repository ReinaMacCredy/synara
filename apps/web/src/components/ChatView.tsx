...
import { resolveSupervisedBoundModelSelection } from "../lib/supervisedOrchestration";
...
export default function ChatView({
  ...
  const selectProcessTask = useTaskProcessStore((store) => store.selectTask);
  ...
  const durableSupervisionProfileSnapshot = useMemo(() => {
    ...
    const seat =
      ...
      null;
    return seat?.profileSnapshotId
      ? (supervisedOrchestration.profileSnapshots.find(
          (snapshot) => snapshot.id === seat.profileSnapshotId,
        ) ?? null)
      : null;
  }, [
    isServerThread,
  ...
  const openSessionProgressTask = useCallback(
    (taskId: ProjectTaskId) => {
      ...
      selectProcessTask(sessionProgress.processId, taskId);
      void navigate({
        to: "/process/$processId",
        params: { processId: sessionProgress.processId },
      });
    },
    [navigate, selectProcessTask, sessionProgress],
  );
  ...
  const threadWorkspaceCwd = activeProject
    ...
    : null;
  const threadArtifactWorkspaceRoot = activeProject?.cwd ?? null;
  ...
  useLayoutEffect(() => {
    ...
    // An accepted send keeps its user row anchored after the opening slide.
    // While auto-follow still owns that coordinate, LegendList's anchored end
  ...
  }, [activeThread?.id, composerTranscriptInsetPx, isInactiveSplitPane, tailAnchor, threadId]);
  ...
  const onSend = async (
    ...
    const effectiveModelSelectionForSend = resolveSupervisedBoundModelSelection({
  ...
  const environmentPanelProps: Omit<EnvironmentPanelProps, "open" | "variant"> = {
    ...
    onOpenProgressTask: openSessionProgressTask,
    onOpenProgressProcess: openSessionProgressProcess,
    onToggleDiff,
