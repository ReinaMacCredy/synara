...
import { describeErrorMessage } from "@veylen/shared/errorMessages";
import {
  TERMINAL_MAX_COLS,
  TERMINAL_MAX_ROWS,
  TERMINAL_MIN_COLS,
...
import type { TerminalSessionSnapshot } from "@veylen/contracts";
...
const TERMINAL_PARKING_CONTAINER_ID = "veylen-terminal-parking";

type VeylenTerminalOptions = NonNullable<ConstructorParameters<typeof Terminal>[0]> & {
...
const TERMINAL_CURSOR_STYLE: NonNullable<VeylenTerminalOptions["cursorStyle"]> = "bar";
const TERMINAL_INACTIVE_CURSOR_STYLE: NonNullable<VeylenTerminalOptions["cursorInactiveStyle"]> =
  "bar";
...
function syncTheme(entry: TerminalRuntimeEntry): void {
  ...
  const terminalOptions = entry.terminal.options as VeylenTerminalOptions;
...
export function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
  ...
  const terminalOptions: VeylenTerminalOptions = {
    cursorBlink: true,
