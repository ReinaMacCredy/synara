// FILE: VeylenThreadCreationCard.tsx
// Purpose: End-of-turn recap for threads created through the Veylen MCP harness.
// Layer: Chat transcript UI

import { PROVIDER_DISPLAY_NAMES } from "@veylen/contracts";
import { formatModelDisplayName } from "@veylen/shared/model";
import { memo } from "react";

import type { WorkLogVeylenThreadCreation } from "../../session-logic";
import { ProviderIcon } from "../ProviderIcon";
import { VeylenLogo } from "../VeylenLogo";
import { Button } from "../ui/button";

function threadMeta(thread: WorkLogVeylenThreadCreation["threads"][number]): string {
...
export const VeylenThreadCreationCard = memo(function VeylenThreadCreationCard({
  creation,
  onOpenThread,
}: {
  readonly creation: WorkLogVeylenThreadCreation;
  ...
  return (
    <div
      ...
      data-veylen-thread-creation-card="true"
    >
      <div className="flex min-w-0 items-center gap-3 px-3 py-2.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-background-elevated-secondary)] text-foreground">
          <VeylenLogo className="h-[22px] w-auto" aria-label="Veylen" />
        </span>
