...
import { CheckpointRef, MessageId, ThreadId, TurnId } from "@veylen/contracts";
...
describe("MessagesTimeline", () => {
  ...
  it("labels only the first message when another task created the conversation", async () => {
    ...
    expect(markup).toContain("Sent by Veylen from another thread");
    expect(markup).toContain('aria-label="Open source thread"');
    expect(markup.indexOf("Sent by Veylen from another thread")).toBeLessThan(
  ...
  it("folds work log summaries above the next assistant message footer", async () => {
    ...
    expect(markup).not.toContain(
      `${formatShortTimestamp("2026-03-17T19:12:29.000Z", "locale")} • 2.0s`,
    );
    expect(markup).not.toContain("Work log");
  });
  ...
  it("shows the Veylen mark for every provider-specific tool row shape", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    const claudeMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-veylen-claude",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-veylen-claude",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "dynamic_tool_call",
              toolTitle: "Veylen__veylen_create_thread",
              toolName: "Veylen__veylen_create_thread",
              detail: "Veylen__veylen_create_thread",
    ...
    expect(claudeMarkup).toContain('data-tool-icon="veylen"');
    expect(claudeMarkup).not.toContain('data-tool-icon="mcp"');
    expect(claudeMarkup).toContain("Veylen is creating a thread");
    expect(claudeMarkup).not.toContain("Veylen__veylen_create_thread");
    ...
    const codexMarkup = renderToStaticMarkup(
      <MessagesTimeline
        {...baseProps}
        expandedWorkGroups={{ "entry-inline-veylen-codex": true }}
        timelineEntries={[
          {
            id: "entry-inline-veylen-codex",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-veylen-codex",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "file_change",
              toolTitle: "mcp__Veylen__veylen_list_threads",
              detail: "mcp__Veylen__veylen_list_threads",
            },
          },
        ]}
      />,
    );
    expect(codexMarkup).toContain('data-tool-icon="veylen"');
    expect(codexMarkup).toContain("Veylen listed threads");
    expect(codexMarkup).not.toContain("mcp__Veylen__veylen_list_threads");
    ...
    const failedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-veylen-failed",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-veylen-failed",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
              toolName: "mcp__veylen__veylen_create_threads",
              toolStatus: "failed",
    ...
    expect(failedMarkup).toContain("Veylen couldn&#x27;t create threads");
    expect(failedMarkup).toContain("Claude rejected reasoningEffort");
  });

  // Browser calls get the globe rather than the generic Veylen mark: a browsing
  // row is about a page, and the surface it acted on is the first thing to read.
  it("uses the browser icon and action name for Veylen browser calls", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...makeTimelineBaseProps()}
        timelineEntries={[
          {
            id: "entry-inline-veylen-browser",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-veylen-browser",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
    ...
    expect(markup).not.toContain("Veylen: Browser Open");
  ...
  it("hides raw `ToolName: {json}` argument details behind the humanized heading", async () => {
    ...
    const readThreadMarkup = renderSingleToolRow({
      id: "work-veylen-read-thread-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__veylen__veylen_read_thread",
      detail: 'mcp__veylen__veylen_read_thread: {"threadId":"c357d8c5-b4c1-47d0"}',
      activityKind: "tool.completed",
    });
    expect(readThreadMarkup).toContain("Veylen read a thread");
    expect(readThreadMarkup).not.toContain("mcp__veylen__veylen_read_thread:");
    expect(readThreadMarkup).not.toContain("threadId");

    const diagnoseMarkup = renderSingleToolRow({
      id: "work-veylen-diagnose-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__veylen__veylen_diagnose_thread",
      detail: 'mcp__veylen__veylen_diagnose_thread: {"threadId":"09a1615d-084f-40b9"}',
      activityKind: "tool.completed",
    });
    expect(diagnoseMarkup).toContain("Veylen diagnosed a thread");
    expect(diagnoseMarkup).not.toContain("mcp__veylen__veylen_diagnose_thread:");
    ...
    const dynamicToolMarkup = renderSingleToolRow({
      ...
      detail: 'ToolSearch: {"query":"select:mcp__veylen__veylen_read_thread_events"}',
      activityKind: "tool.completed",
    });
    ...
    expect(failedArgsMarkup).toContain("Veylen couldn&#x27;t create threads");
    expect(failedArgsMarkup).toContain("Invalid params");
  });

  it("keeps Veylen tool calls and adds a thread creation recap at the end of the turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const assistantMessageId = MessageId.makeUnsafe("message-veylen-recap");
    const workEntries = [
      {
        id: "entry-veylen-create-tool",
        kind: "work",
        createdAt: "2026-03-17T19:12:28.000Z",
        entry: {
          id: "work-veylen-create-tool",
          createdAt: "2026-03-17T19:12:28.000Z",
          label: "MCP tool call",
          tone: "tool",
          itemType: "mcp_tool_call",
          toolName: "mcp__veylen__veylen_create_threads",
          toolTitle: "Veylen created threads",
          activityKind: "tool.completed",
        },
      },
      {
        ...
        createdAt: "2026-03-17T19:12:29.000Z",
        entry: {
          id: "work-veylen-create-recap",
          createdAt: "2026-03-17T19:12:29.000Z",
          label: "Created 2 Veylen threads",
          tone: "info",
          activityKind: "veylen.threads.created",
    ...
    expect(liveMarkup).toContain("Veylen created threads");
    expect(liveMarkup).not.toContain('data-veylen-thread-creation-card="true"');
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          ...workEntries,
          {
            id: "entry-veylen-recap-assistant",
            kind: "message",
            createdAt: "2026-03-17T19:12:30.000Z",
            message: {
              id: assistantMessageId,
    ...
    expect(markup).toContain("Worked for");
    expect(markup).toContain('data-veylen-thread-creation-card="true"');
    ...
    expect(markup.indexOf("Both threads are running.")).toBeLessThan(
      markup.indexOf('data-veylen-thread-creation-card="true"'),
    );
  });
          ...
          new Map([
            [
              assistantMessageId,
              {
                ...
                checkpointRef: CheckpointRef.makeUnsafe("refs/veylen/checkpoints/thread/turn/1"),
                status: "ready",
                completedAt: "2026-03-17T19:12:30.000Z",
                assistantMessageId,
                files: [
