...
import { isWorkspaceRelativePathSafe } from "@veylen/shared/path";
import type { ProjectId, ThreadId, TurnId } from "@veylen/contracts";
...
export function SingleChatSurface(props: {
  ...
  const supervisorConversationThreadId = useStore((store) =>
    resolvePrimarySupervisorThreadId(store.supervisedOrchestration.agentSeats ?? []),
  );
  ...
  const [supervisedOperationsRequest, setSupervisedOperationsRequest] = useState<
    (SupervisedTopologyOpenTarget & { readonly requestId: number }) | undefined
  >();
  ...
  const renderRoomConversation = (conversationThreadId: ThreadId, paneScopeId: string) => (
    <SidebarInset
