...
export default function Sidebar() {
  ...
  const selectedSupervisedRoomId = useMemo(() => {
    ...
    const roomId = activeSeat?.roomIds.find((candidate) =>
      supervisedRooms.some((room) => room.id === candidate),
    );
    return roomId ? ThreadId.makeUnsafe(roomId) : null;
  }, [activeSidebarThreadId, supervisedAgentSeats, supervisedRooms]);
  useEffect(() => {
  ...
  function threadActivityState(thread: SidebarThreadSummary): ThreadActivityState {
    const status = resolveThreadStatusForSidebar(thread);
    if (!status) return "idle";
    if (status.pulse) return "working";
    if (status.label === "Completed") return "ready";
    if (status.colorClass.includes("destructive")) return "failed";
    if (status.colorClass.includes("warning") || status.colorClass.includes("amber")) {
      return "blocked";
    }
    return "idle";
  }
  ...
  function renderSupervisedRoomHistoryRow(room: Room) {
    const isActive = pathname === `/supervised/${room.id}` && settingsSectionSearch.view !== "chat";
