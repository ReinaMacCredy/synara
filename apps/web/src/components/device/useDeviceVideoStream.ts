...
import type { DeviceFrame } from "@veylen/shared/deviceFrame";
...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Veylen could not read.");
        return;
      }
      teardownDecoder();
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Veylen could not read."
