...
function recordThreadTurnStartCommand(command: unknown): boolean {
  ...
  let current = fixture.snapshot.threads.find((thread) => thread.id === threadId);
  if (
    !current &&
    "threadBootstrap" in command &&
    command.threadBootstrap &&
    typeof command.threadBootstrap === "object"
  ) {
    recordThreadCreateCommand({
      type: "thread.create",
      threadId,
      ...command.threadBootstrap,
    });
    current = fixture.snapshot.threads.find((thread) => thread.id === threadId);
  }
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("promotes a Primary Supervisor draft in place without showing the conversation loader", async () => {
    ...
    const mounted = await mountChatView({
      ...
      snapshot: snapshotWithSupervisorProfile,
  ...
  it("keeps the settled activity row while an optimistic follow-up starts", async () => {
    ...
    try {
      ...
      const settledActivityRow = document.querySelector<HTMLElement>(
        '[data-turn-work-region="turn-activity:msg-user-21"]',
      );
      expect(settledActivityRow).not.toBeNull();
