import type { GitBranch, ProviderKind } from "@veylen/contracts";
import {
...
} from "@veylen/shared/composerSlashCommands";
...
> = {
  ...
  feedback: {
    command: "feedback",
    label: "/feedback",
    description: "Send feedback to the Veylen team",
    source: "app",
  },
  automation: {
    command: "automation",
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.provider !== "claudeAgent"
      ...
      : [
          ...
          // creates a Veylen split/context clone before the provider sees the first turn.
          // /export is app-level too — Veylen owns the thread transcript, so the download
