...
import { BROWSER_TOOL_NAMES } from "@veylen/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      VEYLEN_HOME: home,
      VEYLEN_BROWSER_HOST_PIPE_PATH: pipePath,
      VEYLEN_BROWSER_HOST_CAPABILITY: capability,
      VEYLEN_E2E_SHELL_PATH: shellPath,
      VEYLEN_E2E_THREAD_ID: threadId,
    },
  });
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __veylenVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__veylenVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__veylenVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __veylenVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
      ).__veylenVisibleBrowserE2E.browserManager;
