import type { ExternalMcpCapability } from "@veylen/contracts";
...
export function buildExternalMcpOverviewNextSteps(
  ...
  return [
    ...
    ...(capabilities.has("tasks:create") ? ["Create work with veylen_create_task."] : []),
    ...(capabilities.has("tasks:wait") ? ["Follow permitted work with veylen_wait_for_task."] : []),
    ...(capabilities.has("tasks:read")
      ? ["Read permitted task results with veylen_read_task."]
      : []),
  ];
}
