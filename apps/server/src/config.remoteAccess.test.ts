...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without VEYLEN_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://veylen.example.test/"),
      new URL("https://veylen.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://veylen.example.test/"))?.origin).toBe(
      "https://veylen.example.test",
    );
    for (const value of [
      "http://veylen.example.test/",
      "https://user:pass@veylen.example.test/",
      "https://veylen.example.test/app",
      "https://veylen.example.test/?query=1",
      "https://veylen.example.test/#fragment",
    ]) {
