...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "VEYLEN_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://veylen.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Veylen through VEYLEN_PUBLIC_URL/--public-url without VEYLEN_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Veylen to non-loopback host ${config.host ?? "<unspecified>"} without VEYLEN_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with VEYLEN_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with VEYLEN_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export function resolveDefaultChatWorkspaceRoot(input: {
  ...
  return pathApi.join(homeDir, "Documents", "Veylen");
}
 ...
 * both via {@link realpathNearestExisting}.
 *
 ...
 * Otherwise a symlinked chat ancestor can make client-side classifiers
 * mis-detect which container a thread belongs to.
 */
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.VEYLEN_STATIC_DIR?.trim();
  if (snapshotDir) {
