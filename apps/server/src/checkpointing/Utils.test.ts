...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Veylen refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/veylen\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/veylen/", "refs/historical/");
