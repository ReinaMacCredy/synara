...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("veylen-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("veylen-codex-home-");
  ...
  it("allows images written to the VEYLEN_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Veylen overlay
    // directory (see resolveVeylenCodexHomeOverlayPath). Generated images therefore
    // live under <VEYLEN_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const veylenHome = path.join(fakeRoot, "veylen", "runtime");
    ...
    process.env.VEYLEN_HOME = veylenHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("veylen-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/veylen-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "veylen-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("veylen-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("veylen-image-tmp-root-");
