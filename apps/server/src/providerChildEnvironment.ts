...
// Purpose: Builds provider child environments without Veylen control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("VEYLEN_FAKE_") || key.startsWith("VEYLEN_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedVeylenKeys?: ReadonlyArray<string>;
  ...
  const allowedVeylenKeys = new Set(input.inheritedVeylenKeys ?? []);
  ...
  for (const [key, value] of Object.entries(baseEnv)) {
    if (
      key.startsWith("VEYLEN_") &&
      !allowedVeylenKeys.has(key) &&
