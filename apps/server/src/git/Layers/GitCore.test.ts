  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("veylen: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "veylen: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "veylen/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "veylen/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "veylen/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "veylen/tmp-working",
          newBranch: "veylen/feat/session",
        });

        expect(renamed.branch).toBe("veylen/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "veylen/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "veylen/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("veylen/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "veylen/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "veylen/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "veylen/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "veylen/tmp-working" });
        ...
        expect(renamed.branch).toBe("veylen/feat/session-2");
      }),
    );
  ...
  describe("createGitWorktree", () => {
    ...
    it.effect("creates a branch-backed managed worktree when newBranch is provided", () =>
      Effect.gen(function* () {
        ...
        const result = yield* core.createDetachedWorktree({
          ...
          ref: "HEAD",
          path: wtPath,
          newBranch: "veylen/abcd1234",
        });

        expect(result.worktree).toEqual({
          path: wtPath,
          ref: expectedHead,
          branch: "veylen/abcd1234",
        });
        expect(yield* git(wtPath, ["symbolic-ref", "--short", "HEAD"])).toBe("veylen/abcd1234");
        expect(yield* git(tmp, ["rev-parse", "refs/heads/veylen/abcd1234"])).toBe(expectedHead);
        ...
        const remainingBranches = yield* git(tmp, ["branch", "--list", "veylen/abcd1234"]);
    ...
    it.effect("does not report branch creation before preliminary validation", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["branch", "--list", "veylen/notstarted"])).toBe("");
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const result = yield* Effect.exit(
          core.createDetachedWorktree({
            ...
            ref: "HEAD",
            path: wtPath,
            newBranch: "veylen/rollback1",
          }),
        );

        expect(Exit.isFailure(result)).toBe(true);
        expect(yield* git(tmp, ["branch", "--list", "veylen/rollback1"])).toBe("");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:example-org/veylen.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:example-org/veylen.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "veylen/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
