...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "veylen-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "done",
        'stdin_content="$(cat)"',
        'if [ "$VEYLEN_FAKE_CODEX_REQUIRE_IMAGE" = "1" ] && [ "$seen_image" != "1" ]; then',
        '  printf "%s\\n" "missing --image input" >&2',
        "  exit 2",
        "fi",
        'if [ "$VEYLEN_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK" = "1" ] && [ "$seen_skip_git_repo_check" != "1" ]; then',
        '  printf "%s\\n" "missing --skip-git-repo-check" >&2',
        "  exit 9",
        "fi",
        'if [ "$VEYLEN_FAKE_CODEX_REQUIRE_APPROVAL_NEVER" = "1" ] && [ "$seen_approval_never" != "1" ]; then',
        '  printf "%s\\n" "missing approval_policy=never" >&2',
        "  exit 10",
        "fi",
        'if [ -n "$VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN" ]; then',
        '  printf "%s" "$stdin_content" | grep -F -- "$VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "stdin missing expected content" >&2',
        "    exit 3",
        "  }",
        "fi",
        'if [ -n "$VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" ]; then',
        '  if printf "%s" "$stdin_content" | grep -F -- "$VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" >/dev/null; then',
        '    printf "%s\\n" "stdin contained forbidden content" >&2',
        "    exit 4",
        "  fi",
        "fi",
        'if [ "$VEYLEN_FAKE_CODEX_REQUIRE_CODEX_HOME" = "1" ] && [ -z "$CODEX_HOME" ]; then',
        ...
        "  exit 6",
        "fi",
        'if [ -n "$VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" ]; then',
        '  grep -F -- "$VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null || {',
        '    printf "%s\\n" "CODEX_HOME config missing expected content" >&2',
        "    exit 7",
        "  }",
        "fi",
        'if [ -n "$VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" ]; then',
        '  if grep -F -- "$VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null; then',
        '    printf "%s\\n" "CODEX_HOME config contained forbidden content" >&2',
        "    exit 8",
        "  fi",
        "fi",
        'if [ -n "$VEYLEN_FAKE_CODEX_STDERR" ]; then',
        '  printf "%s\\n" "$VEYLEN_FAKE_CODEX_STDERR" >&2',
        "fi",
        'if [ -n "$output_path" ]; then',
        '  node -e \'const fs=require("node:fs"); const value=process.argv[2] ?? ""; fs.writeFileSync(process.argv[1], Buffer.from(value, "base64"));\' "$output_path" "${VEYLEN_FAKE_CODEX_OUTPUT_B64:-e30=}"',
        "fi",
        'exit "${VEYLEN_FAKE_CODEX_EXIT_CODE:-0}"',
        "",
      ].join("\n"),
    );
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "veylen-codex-text-" });
      const binDir = yield* makeFakeCodexBinary(tempDir);
      const previousPath = process.env.PATH;
      const previousVeylenHome = process.env.VEYLEN_HOME;
      const previousOutput = process.env.VEYLEN_FAKE_CODEX_OUTPUT_B64;
      const previousExitCode = process.env.VEYLEN_FAKE_CODEX_EXIT_CODE;
      const previousStderr = process.env.VEYLEN_FAKE_CODEX_STDERR;
      const previousRequireImage = process.env.VEYLEN_FAKE_CODEX_REQUIRE_IMAGE;
      ...
      const previousRequireApprovalNever = process.env.VEYLEN_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
      const previousCodexHomeConfigMustContain =
        process.env.VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
      const previousCodexHomeConfigMustNotContain =
        process.env.VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;

      yield* Effect.sync(() => {
        process.env.PATH = `${binDir}:${previousPath ?? ""}`;
        process.env.VEYLEN_HOME = tempDir;
        process.env.VEYLEN_FAKE_CODEX_OUTPUT_B64 = Buffer.from(input.output, "utf8").toString(
          "base64",
        );

        if (input.exitCode !== undefined) {
          process.env.VEYLEN_FAKE_CODEX_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.VEYLEN_FAKE_CODEX_STDERR = input.stderr;
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_STDERR;
        }

        if (input.requireImage) {
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_IMAGE = "1";
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_REQUIRE_IMAGE;
        }

        if (input.stdinMustContain !== undefined) {
          process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN;
        }

        if (input.stdinMustNotContain !== undefined) {
          process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN = input.stdinMustNotContain;
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        }

        if (input.requireCodexHome) {
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_CODEX_HOME = "1";
        } else {
        ...
        if (input.requireAuthJson) {
          ...
          delete process.env.VEYLEN_FAKE_CODEX_REQUIRE_AUTH_JSON;
        }

        if (input.requireSkipGitRepoCheck) {
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK = "1";
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
        }

        if (input.requireApprovalNever) {
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_APPROVAL_NEVER = "1";
        } else {
          delete process.env.VEYLEN_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
        }
        ...
        if (input.codexHomeConfigMustContain !== undefined) {
          ...
          delete process.env.VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
        }
        ...
        if (input.codexHomeConfigMustNotContain !== undefined) {
          ...
          delete process.env.VEYLEN_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;
        }
      });
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousVeylenHome === undefined) {
          delete process.env.VEYLEN_HOME;
        } else {
          process.env.VEYLEN_HOME = previous.previousVeylenHome;
        }

        if (previous.previousOutput === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_OUTPUT_B64;
        } else {
          process.env.VEYLEN_FAKE_CODEX_OUTPUT_B64 = previous.previousOutput;
        }

        if (previous.previousExitCode === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_EXIT_CODE;
        } else {
          process.env.VEYLEN_FAKE_CODEX_EXIT_CODE = previous.previousExitCode;
        }

        if (previous.previousStderr === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_STDERR;
        } else {
          process.env.VEYLEN_FAKE_CODEX_STDERR = previous.previousStderr;
        }

        if (previous.previousRequireImage === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_REQUIRE_IMAGE;
        } else {
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_IMAGE = previous.previousRequireImage;
        }

        if (previous.previousStdinMustContain === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN;
        } else {
          process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_CONTAIN = previous.previousStdinMustContain;
        }

        if (previous.previousStdinMustNotContain === undefined) {
          delete process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        } else {
          process.env.VEYLEN_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN =
        ...
        if (previous.previousRequireAuthJson === undefined) {
          ...
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_AUTH_JSON = previous.previousRequireAuthJson;
        }
        ...
        if (previous.previousRequireApprovalNever === undefined) {
          ...
          process.env.VEYLEN_FAKE_CODEX_REQUIRE_APPROVAL_NEVER =
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGenerationLive", (it) => {
  ...
  it.effect("reports a missing working directory before spawning Codex", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parent = yield* fs.makeTempDirectoryScoped({ prefix: "veylen-missing-cwd-" });
  ...
  it.effect("preserves missing Codex binary errors when the working directory exists", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "veylen-existing-cwd-" });
  ...
  it.effect("uses the provided codexHomePath and strips local skills config", () =>
    withFakeCodexEnv(
      ...
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const wrongCodexHome = yield* fs.makeTempDirectoryScoped({ prefix: "veylen-wrong-codex-" });
        const customCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "veylen-custom-codex-",
        });
