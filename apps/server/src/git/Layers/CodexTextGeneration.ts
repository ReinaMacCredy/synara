...
import { DEFAULT_GIT_TEXT_GENERATION_MODEL } from "@veylen/contracts";
import { sanitizeGeneratedThreadTitle } from "@veylen/shared/chatThreads";
import { resolveCodexHome } from "@veylen/shared/codexConfig";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@veylen/shared/git";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `veylen-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `veylen-codex-home-${process.pid}-${randomUUID()}`,
      );
