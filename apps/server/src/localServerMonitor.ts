...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "VeylenLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  const server: ServerLocalServerProcess = {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Veylen cannot signal this process." }),
  };
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Veylen cannot stop this process.",
    };
  }
