...
} from "@veylen/shared/migrationRecovery";
export {
  migrationBackupDirectory,
  migrationRecoveryMarkerPath,
} from "@veylen/shared/migrationRecovery";
...
export class MigrationRecoveryRequiredError extends Error {
  ...
  constructor(
    ...
    super(
      `Migration recovery is required for ${dbPath}.${detail ? ` ${detail}` : ""} Stop every Veylen process, then run: veylen-restore-migration-backup ${shellQuote(dbPath)}`,
    );
  ...
  constructor(
    ...
    super(
      `Not enough free disk space to back up the database before upgrading it. ` +
        ...
        `${formatBytes(availableBytes)} is free. Free up disk space and start Veylen again.`,
    );
...
const writeRecoveryMarker = (dbPath: string, backup: MigrationBackupResult) =>
  attemptPromise(async () => {
    await writeRecoveryMarkerFile(migrationRecoveryMarkerPath(dbPath), {
      ...
      resumeAttempts: 0,
      recovery:
        "Veylen retries an interrupted migration a bounded number of times on startup. Once that budget is spent it refuses to open this database until an operator stops every Veylen process and runs the explicit migration-backup restore command.",
    });
  });
...
/**
 * Explicit one-shot recovery path. The operator must stop every Veylen process
