...
import Migration0098 from "./Migrations/098_SupervisedModelSessions.ts";
import Migration0099 from "./Migrations/099_RetireOrchestratorControlPlane.ts";
...
export const migrationEntries = [
  ...
  [98, "SupervisedModelSessions", Migration0098],
  [99, "RetireOrchestratorControlPlane", Migration0099],
...
/**
 * A tracker identity that a *released* Veylen build wrote for a migration whose
 ...
 * does the migration Veylen registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Veylen's latest ID,
 * every Veylen migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Veylen's lineage and
 ...
 *
 * Divergences caused by Veylen renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Veylen build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Veylen migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Veylen build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Veylen lineage (legacy import); re-running migrations from the divergence point",
