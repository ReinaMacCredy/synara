...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  // Simulates a legacy ~/.veylen import where the imported `effect_sql_migrations`
  // tracker has IDs 17-31 recorded under unrelated Veylen names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported Veylen DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the schema to where Veylen and Veylen last agreed.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under Veylen's old names so the migrator
      // skips Veylen's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the Veylen-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh Veylen install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
