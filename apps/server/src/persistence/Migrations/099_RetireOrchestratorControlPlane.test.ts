...
layer("migration 099", (it) => {
  it.effect(
    "permanently purges populated Orchestrator state and preserves Normal and Supervised state",
    () =>
      Effect.gen(function* () {
      ...
      `;
        ...
        assert.strictEqual(
          ...
          }>`SELECT COUNT(*) AS count FROM projection_supervised_rooms`)[0]?.count,
          1,
        );
