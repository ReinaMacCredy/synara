...
import { MODEL_OPTIONS_BY_PROVIDER } from "@veylen/contracts";
...
// Imported instance ids may be runtime names rather than Veylen provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Veylen writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Veylen stores canonical provider/options objects.
