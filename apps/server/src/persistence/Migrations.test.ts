...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Veylen's latest migration ID. The migrator's max-ID
  // gate then skips every Veylen migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Veylen's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Veylen's latest ID.
      const latestVeylenId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestVeylenId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Veylen lineage exactly; foreign rows are gone.
  ...
  it.effect("refuses writable migration startup for a newer Veylen schema", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureVeylenMigration')
      `;
  ...
  it.effect("keeps a private database that already recorded old migration 54 compatible", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(executed, [
        ...
        [90, "OrchestratorMode"],
        [91, "OrchestratorChildResults"],
        [92, "SupervisedOrchestration"],
        [93, "SupervisionPeerBindings"],
      ...
      assert.deepStrictEqual(tracker.slice(-60), [
        ...
        { migration_id: 90, name: "OrchestratorMode" },
        { migration_id: 91, name: "OrchestratorChildResults" },
        { migration_id: 92, name: "SupervisedOrchestration" },
        { migration_id: 93, name: "SupervisionPeerBindings" },
