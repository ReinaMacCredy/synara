...
import {
  ...
  VEYLEN_TERMINAL_HOOK_OSC_PREFIX,
  VEYLEN_TERMINAL_CLI_KIND_ENV_KEY,
  type TerminalAgentHookEventType,
  type ManagedTerminalCliKind,
} from "@veylen/shared/terminalThreads";
...
function buildHookOscSequence(eventType: TerminalAgentHookEventType): string {
  return `\\033]${VEYLEN_TERMINAL_HOOK_OSC_PREFIX}${eventType}\\007`;
}
...
set -eu
if [ "$#" -gt 0 ]; then
  _veylen_hook_input="$1"
else
  _veylen_hook_input="$(cat)"
fi

_veylen_extract_event() {
  printf '%s' "$_veylen_hook_input" | sed -n "s/.*\\\"$1\\\"[[:space:]]*:[[:space:]]*\\\"\\([^\\\"]*\\)\\\".*/\\1/p" | head -n 1
}

_veylen_event="$(_veylen_extract_event hook_event_name)"
if [ -z "$_veylen_event" ]; then
  _veylen_type="$(_veylen_extract_event type)"
  case "$_veylen_type" in
    ...
    exec_approval_request|apply_patch_approval_request|request_user_input)
      _veylen_event="PermissionRequest"
      ;;
  esac
fi
...
_veylen_emit_osc() {
  _veylen_sequence="$1"
  if [ -w /dev/tty ]; then
    printf '%b' "$_veylen_sequence" > /dev/tty 2>/dev/null || printf '%b' "$_veylen_sequence"
    return
  fi
  printf '%b' "$_veylen_sequence"
}

case "$_veylen_event" in
  UserPromptSubmit|PostToolUse|PostToolUseFailure|Start)
    _veylen_emit_osc '${buildHookOscSequence("Start")}'
    ;;
  Stop)
    _veylen_emit_osc '${buildHookOscSequence("Stop")}'
    ;;
  PermissionRequest|PreToolUse|Notification)
    _veylen_emit_osc '${buildHookOscSequence("PermissionRequest")}'
    ;;
esac
`;
}
...
function buildCodexWrapperScript(input: {
  ...
  return [
    ...
    '    _veylen_codex_ts="$(date +%s 2>/dev/null || echo "$$")"',
    '    export CODEX_TUI_SESSION_LOG_PATH="${TMPDIR:-/tmp}/veylen-codex-session-$$_${_veylen_codex_ts}.jsonl"',
    "  fi",
    "  (",
    '    _veylen_log="$CODEX_TUI_SESSION_LOG_PATH"',
    `    _veylen_notify=${shellQuote(notifyHookPath)}`,
    '    _veylen_last_turn_id=""',
    '    _veylen_last_approval_id=""',
    '    _veylen_last_exec_call_id=""',
    "    _veylen_approval_fallback_seq=0",
    "",
    "    _veylen_emit_event() {",
    '      _veylen_event="$1"',
    `      _veylen_payload=$(printf '{"hook_event_name":"%s"}' "$_veylen_event")`,
    '      "$_veylen_notify" "$_veylen_payload" >/dev/null 2>&1 || true',
    "    }",
    "",
    "    _veylen_i=0",
    '    while [ ! -f "$_veylen_log" ] && [ "$_veylen_i" -lt 200 ]; do',
    "      _veylen_i=$((_veylen_i + 1))",
    "      sleep 0.05",
    "    done",
    '    if [ ! -f "$_veylen_log" ]; then',
    "      exit 0",
    "    fi",
    "",
    '    tail -n 0 -F "$_veylen_log" 2>/dev/null | while IFS= read -r _veylen_line; do',
    '      case "$_veylen_line" in',
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"task_started"'*)`,
    `          _veylen_turn_id=$(printf '%s\n' "$_veylen_line" | awk -F'"turn_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          [ -n "$_veylen_turn_id" ] || _veylen_turn_id="task_started"',
    '          if [ "$_veylen_turn_id" != "$_veylen_last_turn_id" ]; then',
    '            _veylen_last_turn_id="$_veylen_turn_id"',
    '            _veylen_emit_event "Start"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"'*'_approval_request"'*)`,
    `          _veylen_approval_id=$(printf '%s\n' "$_veylen_line" | awk -F'"id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_veylen_approval_id" ] || _veylen_approval_id=$(printf '%s\n' "$_veylen_line" | awk -F'"approval_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_veylen_approval_id" ] || _veylen_approval_id=$(printf '%s\n' "$_veylen_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -z "$_veylen_approval_id" ]; then',
    "            _veylen_approval_fallback_seq=$((_veylen_approval_fallback_seq + 1))",
    '            _veylen_approval_id="approval_request_${_veylen_approval_fallback_seq}"',
    "          fi",
    '          if [ "$_veylen_approval_id" != "$_veylen_last_approval_id" ]; then',
    '            _veylen_last_approval_id="$_veylen_approval_id"',
    '            _veylen_emit_event "PermissionRequest"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"exec_command_begin"'*)`,
    `          _veylen_exec_call_id=$(printf '%s\n' "$_veylen_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -n "$_veylen_exec_call_id" ]; then',
    '            if [ "$_veylen_exec_call_id" != "$_veylen_last_exec_call_id" ]; then',
    '              _veylen_last_exec_call_id="$_veylen_exec_call_id"',
    '              _veylen_emit_event "Start"',
    "            fi",
    "          else",
    '            _veylen_emit_event "Start"',
    ...
    "    done",
    "  ) &",
    "  VEYLEN_CODEX_START_WATCHER_PID=$!",
    "fi",
    `${shellQuote(targetPath)} --enable codex_hooks -c ${shellQuote(`notify=["bash",${JSON.stringify(notifyHookPath)}]`)} "$@"`,
    "_veylen_status=$?",
    'if [ -n "${VEYLEN_CODEX_START_WATCHER_PID:-}" ]; then',
    '  kill "$VEYLEN_CODEX_START_WATCHER_PID" >/dev/null 2>&1 || true',
    '  wait "$VEYLEN_CODEX_START_WATCHER_PID" 2>/dev/null || true',
    "fi",
    'exit "$_veylen_status"',
  ].join("\n");
}
...
function buildWrapperScript(input: {
  ...
  return [
    "#!/bin/sh",
    `# Managed ${commandName} wrapper injected by veylen terminal sessions.`,
    `printf '\\033]0;%s\\007' ${shellQuote(title)}`,
    `export ${VEYLEN_TERMINAL_CLI_KIND_ENV_KEY}=${shellQuote(cliKind)}`,
    commandBody,
    "",
  ].join("\n");
}
...
function buildManagedZshRc(quotedZshDir: string): string {
  return `# Veylen zsh rc wrapper
_veylen_home="\${VEYLEN_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_veylen_home"
[[ -f "$_veylen_home/.zshrc" ]] && source "$_veylen_home/.zshrc"
export ZDOTDIR=${quotedZshDir}
if [ -n "\${VEYLEN_MANAGED_BIN_DIR:-}" ] && [ -d "\${VEYLEN_MANAGED_BIN_DIR}" ]; then
  case ":$PATH:" in
    *:\${VEYLEN_MANAGED_BIN_DIR}:*) ;;
    *) export PATH="\${VEYLEN_MANAGED_BIN_DIR}:$PATH" ;;
  esac
  unalias claude 2>/dev/null || true
  claude() {
    if [ -x "\${VEYLEN_MANAGED_BIN_DIR}/claude" ] && [ ! -d "\${VEYLEN_MANAGED_BIN_DIR}/claude" ]; then
      "\${VEYLEN_MANAGED_BIN_DIR}/claude" "$@"
    else
      command claude "$@"
    fi
  }
  ...
  codex() {
    if [ -x "\${VEYLEN_MANAGED_BIN_DIR}/codex" ] && [ ! -d "\${VEYLEN_MANAGED_BIN_DIR}/codex" ]; then
      "\${VEYLEN_MANAGED_BIN_DIR}/codex" "$@"
    else
      command codex "$@"
    fi
  }
  typeset -ga precmd_functions 2>/dev/null || true
  _veylen_ensure_managed_bin() {
    case ":$PATH:" in
      *:\${VEYLEN_MANAGED_BIN_DIR}:*) ;;
      *) PATH="\${VEYLEN_MANAGED_BIN_DIR}:$PATH" ;;
    esac
  }
  {
    precmd_functions=(\${precmd_functions:#_veylen_ensure_managed_bin} _veylen_ensure_managed_bin)
  } 2>/dev/null || true
fi
`;
}
...
function ensureManagedZshWrappers(zshDir: string): void {
  ...
  writeFileIfChanged(
    ...
    `# Veylen zsh env wrapper
_veylen_home="\${VEYLEN_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_veylen_home"
[[ -f "$_veylen_home/.zshenv" ]] && source "$_veylen_home/.zshenv"
export ZDOTDIR=${quotedZshDir}
`,
    PRIVATE_FILE_MODE,
  );
  writeFileIfChanged(
    ...
    `# Veylen zsh profile wrapper
_veylen_home="\${VEYLEN_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_veylen_home"
[[ -f "$_veylen_home/.zprofile" ]] && source "$_veylen_home/.zprofile"
export ZDOTDIR=${quotedZshDir}
`,
    PRIVATE_FILE_MODE,
  );
...
function applyManagedTerminalWrapperEnvState(
  ...
  return {
    ...env,
    VEYLEN_MANAGED_BIN_DIR: wrapperState.binDir,
    VEYLEN_ORIGINAL_ZDOTDIR: env.ZDOTDIR ?? env.HOME ?? "",
