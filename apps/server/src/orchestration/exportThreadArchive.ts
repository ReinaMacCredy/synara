...
// Purpose: Build a ZIP archive that exports a single Veylen thread so a user
...
// The export-eligibility guard lives in @veylen/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@veylen/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `veylen-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
