 ...
 * fact (for example, a project workspace root prior to being scaffolded) while
 * still matching what `realpath` will return once the directory exists. This
 * is exactly what stored and reported roots must agree on for downstream
 * classifiers such as `isHomeChatContainerProject`.
 */
