 ...
 * directory so both land in the same place under a custom VEYLEN_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.VEYLEN_HOME?.trim() || path.join(homedir(), ".veylen");
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Veylen run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Veylen booted and leaves the
