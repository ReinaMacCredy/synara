 ...
 * - The Veylen boot cap (`DEVICE_VEYLEN_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Veylen-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Veylen's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly veylenBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_VEYLEN_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.veylenBooted]).catch(() => undefined);
  }
  ...
  async veylenBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileVeylenBooted(devices);
    return devices
      .filter((device) => this.veylenBooted.has(device.udid))
   ...
   *
   * The set is Veylen's own bookkeeping, but the simulators are not Veylen's to
   ...
   */
  private reconcileVeylenBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.veylenBooted) {
      if (running.has(udid)) continue;
      this.veylenBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Veylen doing it still held their slots, so
    ...
    // Veylen from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.veylenBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        veylenBooted: await this.veylenBootedDevices(),
      };
    }
    ...
    // exists to stop Veylen accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.veylenBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "veylen" } };
  }
  ...
  /**
   * App quit: shut down everything Veylen booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.veylenBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.veylenBooted.has(device.udid) ? "veylen" : device.bootSource;
   ...
   * the device down or start the idle countdown if Veylen booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Veylen-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.veylenBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.veylenBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
