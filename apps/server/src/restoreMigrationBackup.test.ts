...
describe("migration backup recovery CLI", () => {
  it("ships the recovery command from the bundled server package", async () => {
    ...
    expect(packageJson.bin?.["veylen-restore-migration-backup"]).toBe(
      "dist/restoreMigrationBackup.mjs",
    );
  });

  it("rejects relative database paths and warns operators to stop Veylen", async () => {
    ...
    expect(capture.errors.join("\n")).toContain("veylen-restore-migration-backup");
    expect(capture.warnings.join("\n")).toContain("Stop every Veylen process");
