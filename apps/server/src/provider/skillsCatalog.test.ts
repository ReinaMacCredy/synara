...
import type { ProviderSkillDescriptor } from "@veylen/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "veylen-skills-catalog-"));
  homeDir = path.join(root, "home");
  veylenBaseDir = path.join(homeDir, ".veylen");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Veylen skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, veylenBaseDir });
    await expect(access(path.join(veylenBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from veylen and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(veylenBaseDir, "skills", "portable"), "portable", "Veylen skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, veylenBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("veylen");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, veylenBaseDir });
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, veylenBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Veylen for that provider", async () => {
    await writeSkill(path.join(veylenBaseDir, "skills", "shared"), "shared", "Veylen copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(path.join(veylenBaseDir, "skills", "only-veylen"), "only-veylen", "Fallback");

    const codexView = await discoverSkillsCatalog({ homeDir, veylenBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-veylen")).toBe(true);

    // A provider without its own copy resolves the Veylen fallback.
    ...
    expect(claudeShared?.scope).toBe("veylen");
  });

  it("uses documented provider alias roots before Veylen fallbacks", async () => {
    await writeSkill(path.join(veylenBaseDir, "skills", "shared"), "shared", "Veylen copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(veylenBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, veylenBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(veylenBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, veylenBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, veylenBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .veylen skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".veylen", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(veylenBaseDir, "skills", "portable"), "portable", "Veylen");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("veylen");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(veylenBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(veylenBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "veylen"), descriptor("extra", "veylen")],
    });
