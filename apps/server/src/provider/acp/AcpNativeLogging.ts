import type { ProviderKind, ThreadId } from "@veylen/contracts";
...
const SENSITIVE_ENTRY_NAMES = new Set(["authorization", "veylen_agent_gateway_token"]);

function redactSecretString(value: string): string {
  return value
    .replace(/(\bBearer\s+)[^\s"',}\]]+/gi, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/(VEYLEN_AGENT_GATEWAY_TOKEN\s*=\s*)[^\s"',}\]]+/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/("VEYLEN_AGENT_GATEWAY_TOKEN"\s*:\s*")[^"]*/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(
      /("name"\s*:\s*"VEYLEN_AGENT_GATEWAY_TOKEN"\s*,\s*"value"\s*:\s*")[^"]*/g,
