...
const VEYLEN_PLAN_REVIEW_FEEDBACK =
...
/**
 * Veylen owns the approval step after the planning turn settles. Returning a
...
export function makeGrokExitPlanModeCapturedResponse(): {
  ...
  return {
    outcome: "cancelled",
    feedback: VEYLEN_PLAN_REVIEW_FEEDBACK,
  };
}

/** Veylen has already received an implementation turn, so Grok may leave its native gate. */
