...
//          that makes Veylen catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@veylen/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.veylen/skills`, which Veylen registers at session start
    ...
    case "cursor":
      ...
      // only Veylen-owned paths need inlining.
      return segments.has(".veylen");
    case "claudeAgent":
