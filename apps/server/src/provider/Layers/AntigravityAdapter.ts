...
import {
  ...
  VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV,
  VEYLEN_AGENT_GATEWAY_URL_ENV,
...
/**
 * Hook output when capture is inactive (the session is not Veylen-managed).
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined VEYLEN_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${VEYLEN_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.VEYLEN_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.VEYLEN_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "veylen-capture",
        description: "Streams Antigravity CLI lifecycle events to Veylen when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [VEYLEN_AGENT_GATEWAY_URL_ENV, VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [VEYLEN_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
  return buildProviderChildEnvironment({
    ...
    inheritedVeylenKeys: [
      ...
      "VEYLEN_ANTIGRAVITY_HOOK_DECISION",
      ...gatewayKeys,
    ],
    overrides: {
      VEYLEN_ANTIGRAVITY_EVENTS: input.eventFile,
...
export function buildAntigravityTurnPrompt(
  ...
  const harnessPolicy = takeVeylenHarnessPolicyForProviderSession(state, {
    provider: PROVIDER,
...
export function parseAntigravityCliModelLabel(
  ...
  // Veylen never treats `slug\tName` as a single model id at dispatch.
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Veylen capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "veylen-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Veylen gateway credential is no longer active for this provider turn.",
          });
        }
