...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  it.effect("normalizes whitespace in configuration warnings at the provider boundary", () =>
    Effect.gen(function* () {
      ...
      assert.equal(firstEvent.value.payload.path, "mcp_servers.veylen");
    }),
  );
  ...
  it.effect("preserves canonical Host tool names for native tool items", () =>
    Effect.gen(function* () {
      ...
      lifecycleManager.emit("event", {
        id: asEventId("evt-native-child-create"),
        kind: "notification",
        provider: "codex",
        createdAt: new Date().toISOString(),
        method: "item/completed",
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-1"),
        itemId: asItemId("native_tool_1"),
        payload: {
          item: {
            type: "dynamicToolCall",
            id: "native_tool_1",
            tool: "create_child_thread",
            status: "completed",
          },
        },
      } satisfies ProviderEvent);
