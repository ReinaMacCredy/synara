...
import { decodeOutboundJson, decodeOutboundText, outboundHttp } from "@veylen/shared/outboundHttp";
import {
  Cause,
  DateTime,
  Deferred,
...
import { buildAcpVeylenMcpServers } from "../../agentGateway/mcpInjection.ts";
import {
  type VeylenHarnessPolicyDeliveryState,
...
export const takeGrokVeylenHarnessPolicyTextPart = (
...
const GROK_ACP_DEBUG_ENV = "VEYLEN_GROK_ACP_DEBUG";
const VEYLEN_GROK_ACP_DEBUG_ENV = "VEYLEN_GROK_ACP_DEBUG";
...
// override with VEYLEN_GROK_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const GROK_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "VEYLEN_GROK_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const GROK_PLAN_MODE_PROMPT_PREFIX = [
  "Veylen requested Grok's native plan mode.",
...
const GROK_PLAN_GUARD_HOOK_CALLBACK_ID = "veylen-plan-guard";
...
export function buildGrokPromptMeta(interactionMode: ProviderInteractionMode): {
  ...
  // the provider state when Veylen sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Veylen Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
function isGrokAcpDebugEnabled(): boolean {
  return (
    ...
    process.env[VEYLEN_GROK_ACP_DEBUG_ENV] === "1" ||
  ...
  return Effect.gen(function* () {
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            clientInfo: { name: "Veylen", version: "0.0.0" },
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Veylen cancels the
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokVeylenHarnessPolicyTextPart(
          ctx,
              ...
              Effect.gen(function* () {
                ...
                if (terminalPlanMarkdown !== undefined) {
                  ...
                  yield* offerRuntimeEvent(ctx.lifecycleGeneration, {
                    ...
                    raw: {
                      source: "acp.jsonrpc",
                      method: "veylen.grok.terminal-plan-response",
                      payload: result,
                    },
                  });
                }
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Grok ACP version does not advertise session/fork; Veylen will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Grok session has a turn in flight; Veylen will rebuild the fork from its retained transcript.",
          });
        }
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeGrokAcpRuntime({
                ...
                clientInfo: { name: "Veylen Fork", version: "0.0.0" },
                sessionMeta: GROK_SESSION_META,
              });
