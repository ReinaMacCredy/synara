...
import {
  ...
  takeVeylenHarnessPolicyForProviderSession,
} from "../../agentGateway/harnessPolicy.ts";
import { buildOpenCodeMcpServer, VEYLEN_MCP_SERVER_NAME } from "../../agentGateway/mcpInjection.ts";
...
interface OpenCodeSessionContext {
  ...
  /** Permission request ids resolved by Veylen policy and never surfaced to the UI. */
...
const installOpenCodeGatewayMcp = Effect.fn("installOpenCodeGatewayMcp")(function* (input: {
  ...
  const result = yield* runOpenCodeSdk("mcp.add", () =>
    input.client.mcp.add({
      ...
      name: VEYLEN_MCP_SERVER_NAME,
      config: buildOpenCodeMcpServer(input.connection),
    }),
  );
  const status = result.data?.[VEYLEN_MCP_SERVER_NAME];
  ...
  return yield* new OpenCodeRuntimeError({
    ...
    detail:
      status?.status === "failed"
        ? `${input.displayName} Veylen MCP connection failed: ${status.error}`
        : `${input.displayName} Veylen MCP connection did not become ready.`,
  });
});
...
function isMatchingHarnessPolicyDelivery(
  ...
  return (
    ...
    delivery.policyVersion === VEYLEN_HARNESS_POLICY_VERSION &&
    ...
    ...(input.harnessPolicyDelivered
      ? {
          harnessPolicyDelivery: {
            ...
            policyVersion: VEYLEN_HARNESS_POLICY_VERSION,
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "veylen.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "veylen.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "veylen.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "veylen.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "veylen.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "veylen.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              const completed = yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "veylen.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              if (!completed) return;
              yield* emit(context, {
        ...
        switch (event.type) {
          ...
          case "permission.replied": {
            if (context.policyResolvedPermissionIds.has(event.properties.requestID)) {
              // Veylen policy resolved this request; nothing was surfaced to the UI.
              break;
            }
          ...
          // agent loop. Mirror them into Veylen's canonical transcript stream.
                  ...
                  Effect.gen(function* () {
                      ...
                      gatewayControlAvailable = yield* installOpenCodeGatewayMcp({
                        ...
                        Effect.catchCause((cause) =>
                          Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                            Effect.andThen(
                              Effect.logWarning(
                                `${adapterConfig.displayName} could not install thread-scoped Veylen MCP control`,
                                Cause.squash(cause),
                              ),
                            ),
                            Effect.as(false),
                    ...
                    const createSessionId = resumedSessionId
                      ? // A resumed provider may still be executing an interrupted Plan turn.
                        // Install the read-only ruleset until Veylen dispatches a new turn with a
                        ...
                        // without ever emitting a permission request for Veylen to reject.
      ...
      const sendTurnImpl = Effect.fn("sendTurn")(function* (
        ...
        const harnessPolicy = takeVeylenHarnessPolicyForProviderSession(
          {
        ...
        // Always pin Veylen's interaction mode to OpenCode's primary agent.
        // Otherwise a user config with default agent=plan (or a stale options.agent=plan
        // after leaving Veylen plan mode) can trap default turns in plan mode.
        ...
        if (provider === "kilo") {
          ...
          // server rather than by one long-lived HTTP request. Keep Veylen's event
        ...
        Effect.gen(function* () {
          ...
          if (sourceContext?.activeTurnId !== undefined) {
            return yield* new ProviderAdapterValidationError({
              ...
              issue: `The source ${adapterConfig.displayName} session has a turn in flight; Veylen will rebuild the fork from its retained transcript.`,
            });
          }
