...
export const takeCursorVeylenHarnessPolicyTextPart = (
...
// forever. Generous by design; override with VEYLEN_CURSOR_TURN_IDLE_TIMEOUT_MS.
const CURSOR_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "VEYLEN_CURSOR_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const CURSOR_PLAN_MODE_PROMPT_PREFIX = [
  "Veylen Cursor plan mode is active.",
  ...
  return Effect.gen(function* () {
    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeCursorVeylenHarnessPolicyTextPart(
          ctx,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Cursor ACP version does not advertise session/fork; Veylen will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Cursor session has a turn in flight; Veylen will rebuild the fork from its retained transcript.",
          });
        }
