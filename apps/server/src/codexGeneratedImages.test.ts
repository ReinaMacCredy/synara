...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousVeylenHome === undefined) delete process.env.VEYLEN_HOME;
    else process.env.VEYLEN_HOME = previousVeylenHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.VEYLEN_HOME = "/veylen-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/veylen-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.veylen/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.veylen/runtime/codex-home-overlay";
