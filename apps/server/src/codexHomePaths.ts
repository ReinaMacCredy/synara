...
export const VEYLEN_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveVeylenCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.VEYLEN_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".veylen", "runtime");
  return path.join(overlayRoot, VEYLEN_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Veylen keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveVeylenCodexHomeOverlayPath(env, source);
