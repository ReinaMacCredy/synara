...
describe("agent gateway MCP injection", () => {
  ...
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.veylen]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.veylen]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.veylen]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, VEYLEN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, VEYLEN_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, VEYLEN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', VEYLEN_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${VEYLEN_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, VEYLEN_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, VEYLEN_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${VEYLEN_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.veylen]\nurl = "x"', "[mcp_servers.veylen]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [mcp_servers.veylen]  # managed", "[mcp_servers.veylen]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [ mcp_servers.veylen ]  # managed", "[mcp_servers.veylen]"),
    );
    assert.isTrue(configHasTomlTableHeader("  [ mcp_servers . veylen ]", "[mcp_servers.veylen]"));
    assert.isTrue(configHasTomlTableHeader('[mcp_servers."veylen"]', "[mcp_servers.veylen]"));
    assert.isTrue(configHasTomlTableHeader("['mcp_servers'.'veylen']", "[mcp_servers.veylen]"));
    assert.isTrue(configHasTomlTableHeader('[mcp_servers."veyl\\u0065n"]', "[mcp_servers.veylen]"));
    assert.isTrue(
      configHasTomlTableHeader('["shell_environment_policy"]', "[shell_environment_policy]"),
    );
    assert.isFalse(configHasTomlTableHeader('["mcp_servers.veylen"]', "[mcp_servers.veylen]"));
    assert.isFalse(configHasTomlTableHeader('[mcp_servers."syn\\qara"]', "[mcp_servers.veylen]"));
    // A commented-out example block must not count as the table being present.
    assert.isFalse(configHasTomlTableHeader("# [mcp_servers.veylen]", "[mcp_servers.veylen]"));
    assert.isFalse(
      configHasTomlTableHeader('note = "see [mcp_servers.veylen] docs"', "[mcp_servers.veylen]"),
    );
  });
        ...
        result:
          request.method === "tools/list"
            ? {
                tools: [
                  {
                    name: "veylen_list_threads",
                    description: "List Veylen threads.",
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpVeylenMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        args: stdioProxy.args,
        env: [
          { name: "VEYLEN_AGENT_GATEWAY_URL", value: connection.url },
          { name: VEYLEN_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
