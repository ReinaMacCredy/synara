...
export const VEYLEN_HARNESS_POLICY_VERSION = "2026-08-03.8";
export const VEYLEN_HARNESS_POLICY_MARKER = `[Veylen harness policy ${VEYLEN_HARNESS_POLICY_VERSION}]`;

export interface VeylenHarnessCapabilities {
 ...
 * Veylen resources.
 */
export function renderVeylenHarnessPolicy(capabilities: VeylenHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use the veylen_* tools for Veylen threads, projects, automations, and coordination.",
        "Use the browser_* tools autonomously whenever the user refers in any language to Veylen's integrated, embedded, visible, or in-app browser. They are the canonical and complete control surface for that browser: do not load or use a generic Browser, Chrome, Computer Use, OS-automation, Node REPL, Playwright, or other browser-control skill/tool instead. They control the exact thread-scoped Electron page Veylen surfaces to the user, including its live DOM, cookies, and session. The page may continue in the background while the user views another chat; browser actions must never change the user's active chat. When no assigned tab exists, start with browser_open rather than browser_navigate. Take a fresh semantic browser_snapshot before element actions and after navigation or human interaction, requesting an image only when semantics are insufficient.",
        ...
        "For thread discovery and diagnosis, use veylen_list_threads, veylen_read_thread, veylen_read_thread_activity, veylen_read_thread_events, veylen_read_thread_runtime_events, and veylen_diagnose_thread before inspecting Veylen's SQLite files or process logs. Fall back to host storage only when a tool's coverage metadata says the required evidence is unavailable.",
        "Provider-native subagent or Task tools are implementation details: they do not create Veylen threads and must not substitute for an explicit request to create Veylen threads.",
        "For a plural thread request, submit one exact veylen_create_threads plan. The array length is the exact requested count.",
        "If veylen_create_threads rejects the plan during validation or preflight before returning an operationId, correct that same plan and retry it with the same requestId. This is safe because no durable operation, thread, or worktree was created.",
        "Use veylen_capabilities to select canonical provider, model, and option values. Never guess a model slug or silently substitute a provider or model.",
        "Provider option keys are not interchangeable: Codex uses options.reasoningEffort and Claude Agent uses options.effort. Follow veylen_capabilities.targetConstruction for every provider instead of inspecting Veylen source code.",
        "When results are requested, call veylen_wait_for_threads for the created thread ids, wait for every requested result, then synthesize all outcomes.",
        "After veylen_create_threads returns an operationId, retries must keep the same requestId and exact plan. Report terminal operation failures as outcomes; do not create replacement threads unless the user gives a new instruction.",
        "Veylen automations support heartbeat, standalone, and dedicated modes plus interval, once, daily, weekdays, weekly, and cron schedules. Existing everyMinutes heartbeat calls remain supported. Use fastInterval: true only when the user explicitly accepts a sub-minute bounded loop.",
        ...
        'Mode does not restrict stop conditions. completionPolicy {"type":"ai-evaluated","stopWhen":"..."} works in both modes and disables the automation when the clause matches a successful run; prefer it over encoding the stop condition in the prompt. maxIterations remains the backstop, and an automation-dispatched run may always call veylen_cancel_automation on its own automation.',
        AUTOMATION_AUTHORING_GUIDANCE,
        "Prefer veylen_create_automation with suggested: true when the user has not explicitly asked to create an automation. Suggested automations remain disabled until the user accepts their proposal card.",
        "Before veylen_update_automation, call veylen_view_automation and resend the complete mutable configuration, including unchanged fields. Updates are full replacement and partial payloads are rejected.",
        ...
        'During an automation-dispatched turn, persist durable context with veylen_update_automation_memory {"memory": "..."} before finishing; memory is full replacement, DB-backed, and capped at 32 KiB.',
        'Every automation-dispatched turn must finish by calling veylen_report_automation_result. Use decision "silent" only for a successful run with nothing requiring user attention; otherwise use "notify" with a concise title and summary. Failures remain visible regardless of this decision or the automation notification policy. Never call this tool for a manual follow-up turn.',
      ]
    : [
        "Veylen MCP control is unavailable in this provider session. Do not claim that Veylen threads, projects, or automations were created or changed.",
        "Provider-native subagent or Task tools do not create Veylen threads. If the user explicitly requests Veylen resource management, explain that this session cannot perform it.",
      ];

  return [
    VEYLEN_HARNESS_POLICY_MARKER,
    "You are running inside Veylen. Veylen is the host and harness for this session.",
    ...controlPolicy,
  ].join("\n");
}

export const VEYLEN_GATEWAY_HARNESS_POLICY = renderVeylenHarnessPolicy({
  gatewayControlAvailable: true,
});

export const VEYLEN_IDENTITY_ONLY_HARNESS_POLICY = renderVeylenHarnessPolicy({
  gatewayControlAvailable: false,
});

export interface VeylenHarnessPolicyDeliveryState {
  harnessPolicyDelivered?: boolean | undefined;
}

const PROVIDERS_WITH_THREAD_SCOPED_VEYLEN_MCP = new Set<ProviderKind>([
  "codex",
  "claudeAgent",
  "antigravity",
  "cursor",
...
export function providerHasVeylenGatewayControl(input: {
  readonly provider: ProviderKind;
  readonly scopedGatewayConnectionAvailable: boolean;
}): boolean {
  return (
    input.scopedGatewayConnectionAvailable &&
    PROVIDERS_WITH_THREAD_SCOPED_VEYLEN_MCP.has(input.provider)
  );
}

/** Return the private host-context block exactly once for one provider session. */
export function takeVeylenHarnessPolicyForSession(
  state: VeylenHarnessPolicyDeliveryState,
  capabilities: VeylenHarnessCapabilities,
): string | null {
  if (state.harnessPolicyDelivered === true) return null;
  state.harnessPolicyDelivered = true;
  return [
    "<veylen_host_context>",
    renderVeylenHarnessPolicy(capabilities),
    "</veylen_host_context>",
  ].join("\n");
}
 ...
 */
export function takeVeylenHarnessPolicyForProviderSession(
  state: VeylenHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  return takeVeylenHarnessPolicyForSession(state, {
    gatewayControlAvailable: providerHasVeylenGatewayControl(input),
  });
}

export function takeVeylenHarnessPolicyTextPartForProviderSession(
  state: VeylenHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  const text = takeVeylenHarnessPolicyForProviderSession(state, input);
