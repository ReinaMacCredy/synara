/**
 * Stdio-to-HTTP proxy script for the Veylen agent gateway.
 *
...
const STDIO_PROXY_SCRIPT = `// Veylen agent gateway stdio<->HTTP MCP proxy (generated file, do not edit).
const url = process.env.VEYLEN_AGENT_GATEWAY_URL;
let token = process.env.VEYLEN_AGENT_GATEWAY_TOKEN;
let bootstrapToken = process.env.VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside Veylen therefore sees a valid empty MCP server
  // instead of a noisy failed integration. Veylen-managed processes receive
    ...
    return [
      {
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "veylen", version: "1.0.0" },
        },
      },
    ];
  }
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Veylen is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("Veylen gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("Veylen gateway bootstrap returned an invalid response");
      }
      token = payload.bearerToken;
      bootstrapToken = undefined;
      delete process.env.VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
      return token;
    })().finally(() => {
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "Veylen gateway request failed: " + String(error) },
      },
    ];
  }
}
