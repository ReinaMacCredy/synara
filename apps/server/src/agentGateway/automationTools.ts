...
} from "@veylen/shared/automationMode";
...
export function makeAgentGatewayAutomationTools(
  ...
  const createAutomation: ToolEntry = {
    ...
    definition: {
      name: "veylen_create_automation",
      description: `Create a heartbeat, standalone, or dedicated Veylen automation. ${AUTOMATION_AUTHORING_GUIDANCE} Existing calls remain compatible: omitting mode/schedule creates a heartbeat on your thread using everyMinutes (default 5). Prefer suggested:true unless the user explicitly requested creation.`,
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Create a Veylen automation", ...WRITE_TOOL_ANNOTATIONS },
    },
  ...
  const listAutomations: ToolEntry = {
    ...
    definition: {
      name: "veylen_list_automations",
      description:
        "List Veylen automations (id, name, mode, schedule, target thread, enabled, next run).",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List Veylen automations", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args) =>
      Effect.gen(function* () {
  ...
  const viewAutomation: ToolEntry = {
    requiredCapability: "thread:read",
    definition: {
      name: "veylen_view_automation",
      description:
        "View a complete automation definition, recent runs, next run, and persistent-memory excerpt. Call this immediately before veylen_update_automation and resend every unchanged mutable field.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "View a Veylen automation", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const updateAutomation: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "veylen_update_automation",
      description: `Fully replace an automation's mutable configuration. ${AUTOMATION_AUTHORING_GUIDANCE} You MUST call veylen_view_automation first, then resend name, prompt, schedule, enabled, maxIterations, notificationPolicy, and completionPolicy, including every unchanged field. Partial updates are rejected.`,
      inputSchema: {
        type: "object",
        properties: {
          automationId: { type: "string" },
      ...
      annotations: { title: "Replace a Veylen automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const cancelAutomation: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "veylen_cancel_automation",
      description:
        'Stop a Veylen automation. mode "disable" (default) pauses it and keeps history; "delete" archives it. An automation-dispatched run may always stop its own automation, whatever its mode. Prefer a completionPolicy stop clause for conditions known when the automation is created.',
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Stop a Veylen automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const updateMemory: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "veylen_update_automation_memory",
      description:
  ...
  const reportResult: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "veylen_report_automation_result",
      description:
