/**
 * Provider-facing config builders for the Veylen agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.veylen]` TOML block (streamable HTTP +
...
export const VEYLEN_MCP_SERVER_NAME = "veylen";
export const VEYLEN_AGENT_GATEWAY_TOKEN_ENV = "VEYLEN_AGENT_GATEWAY_TOKEN";
export const VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const VEYLEN_AGENT_GATEWAY_URL_ENV = "VEYLEN_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Veylen sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${VEYLEN_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(VEYLEN_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(VEYLEN_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Veylen thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Veylen MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Veylen MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Veylen MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Veylen MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Veylen MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Veylen MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Veylen's Antigravity
  ...
  return {
    mcpServers: {
      [VEYLEN_MCP_SERVER_NAME]: {
        ...
        env: {
          [VEYLEN_AGENT_GATEWAY_URL_ENV]: `$${VEYLEN_AGENT_GATEWAY_URL_ENV}`,
          [VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${VEYLEN_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpVeylenMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: VEYLEN_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: VEYLEN_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
