...
import {
  ...
  type VeylenCreateThreadsResult,
} from "@veylen/contracts";
import { buildPromptThreadTitleFallback } from "@veylen/shared/chatThreads";
import { WORKTREE_BRANCH_PREFIX } from "@veylen/shared/git";
import { parseGitHubRepositoryNameWithOwnerFromPullRequestUrl } from "@veylen/shared/githubRepository";
...
export const makeCreateThreadsHandler = Effect.fn(function* (
  ...
  const awaitCreationReplay = (
    ...
    Effect.gen(function* () {
      ...
      return yield* Effect.fail(
        new GatewayToolError(
          "operation_failed",
          "The original thread-creation operation is still in progress. Retry only with the same request id; Veylen will not create replacement threads.",
  ...
  const appendThreadCreationRecap = (input: {
    ...
    readonly result: VeylenCreateThreadsResult;
  }) => {
    ...
    return orchestrationEngine
      .dispatch({
        ...
        activity: {
          ...
          tone: "info",
          kind: "veylen.threads.created",
          summary: `Created ${input.result.createdCount} Veylen ${threadLabel}`,
          payload: {
            source: "veylen_mcp",
            operationId: input.result.operationId,
  ...
  const run = (input: typeof VeylenCreateThreadsInput.Type, context: GatewayCreationContext) => {
    return Effect.gen(function* () {
      ...
      if (deprecatedBranchName) {
        return yield* Effect.fail(
          new ToolInputError(
            '"branchName" is no longer supported for managed worktrees. Veylen creates a managed temporary branch and renames it after the first prompt; create additional branches inside the new thread if needed.',
          ),
        );
      }
        ...
        Effect.gen(function* () {
          ...
          if (environment === "worktree") {
            ...
            if (existsSync(plannedWorktreePath)) {
              return yield* Effect.fail(
                new ToolInputError(
                  `Worktree path "${plannedWorktreePath}" already exists. Veylen will not reuse or remove a pre-existing path.`,
                ),
              );
            }
          }
          return {
            ...
            // The 8-hex-digit token keeps it a temporary veylen/* branch.
            newBranch:
        ...
        Effect.gen(function* () {
                          ...
                          Effect.flatMap(() =>
                            worktree.branch === null
                              ...
                              : // The branch is this operation's own deterministic
                                // veylen/* name and its worktree was just force-removed.
          ...
          if (compensationErrors.length > 0) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Veylen could not dispatch the exact creation plan and cleanup is still pending. The durable operation remains compensating and will never create replacements.",
          ...
          if (statusFailure !== null) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Veylen compensated the created resources but could not persist a terminal operation status. The operation remains compensating and will never create replacements.",
              { operationId, ...failure, compensationPending: true },
            );
          }
          return new GatewayToolError(
            "operation_failed",
            "Veylen could not dispatch the exact creation plan. Created operation-owned resources were compensated; no replacements were created.",
            { operationId, ...failure },
          );
        });
      ...
      const outcome = yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
                ...
                Effect.gen(function* () {
                  ...
                  yield* orchestrationEngine
                    .dispatch({
                      ...
                      worktreePath,
                      creationSource:
                        context.kind === "external-client" ? "external_mcp" : "veylen_mcp",
                      ...(context.kind === "provider-session"
                        ? {
          ...
          const result = {
            ...
            threads: results,
          } satisfies VeylenCreateThreadsResult;
