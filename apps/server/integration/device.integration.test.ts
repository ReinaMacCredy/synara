/**
 * Device pane end-to-end verification against a live Veylen server.
 *
...
import {
  ...
  decodeDeviceFrame,
} from "@veylen/shared/deviceFrame";
import {
  DEVICE_WS_METHODS,
...
/**
 * Reboot the simulator behind Veylen's back, the way `simctl` from a terminal,
...
async function screenshotViaSimctl(udid: string): Promise<Buffer> {
  const path = join(tmpdir(), `veylen-device-e2e-${randomUUID()}.png`);
  try {
...
describeE2e("device pane end-to-end", () => {
  ...
  it("boots the target simulator", async () => {
    ...
    expect(booted?.bootSource).toBe("veylen");
  }, 180_000);
  ...
  it("keeps input working after the simulator is rebooted outside Veylen", async () => {
    if (!target) throw new Error("no target device");
