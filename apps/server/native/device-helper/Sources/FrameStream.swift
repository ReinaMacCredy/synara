...
// `@veylen/contracts` (see HEADER.md).
//
...
final class FrameSocketWriter {
  ...
  private let queue = DispatchQueue(label: "dev.veylen.device-helper.socket")
...
final class FrameStream {
  ...
  private let encodeQueue = DispatchQueue(label: "dev.veylen.device-helper.encode")
