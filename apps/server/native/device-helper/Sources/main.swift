// veylen-device-helper — the native side of Veylen's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: VeylenHIDBridge?
  private(set) var accessibility: VeylenAXBridge?
  ...
  func requireHID() throws -> VeylenHIDBridge {
  ...
  func requireAccessibility() throws -> VeylenAXBridge {
...
///
/// Every injection path in VeylenHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: VeylenHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = VeylenHardwareButton.home
    guard VeylenHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.veylen.device-helper.rpc")
